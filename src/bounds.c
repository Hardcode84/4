...
#define MODULAR_DELTA_DEPTH_LIMIT 32u
#define MODULAR_DELTA_TERM_LIMIT 32u
#define MODULAR_DELTA_VISIT_LIMIT 4096u
...
static bool bounds_get_proportional_primitive(
  ...
  size_t term_bytes;
  uint32_t i;
  if (!b || !b->ctx || !expr || !primitive || !scale_p || !scale_q ||
      !offset_p || !offset_q)
    return false;
 ...
 * Direct comparisons which could not be normalized earlier are subtracted
 * here; using the relation remains guarded by independent endpoint-definedness
 * proofs at query time. */
static bool bounds_extract_cmp_exact_relation(ixs_bounds *b, ixs_node *cmp,
  ...
  if (constant == INT64_MIN) {
    /* positive == negative + 2^63 is not int64-representable in this
     * orientation.  Store the exact reverse relation instead. */
    *lhs = negative;
    *rhs = positive;
    *offset = INT64_MIN;
  } else {
...
static bool bounds_wide_offset_add(bounds_wide_offset a, bounds_wide_offset b,
  ...
  if (a.negative == b.negative) {
    if (a.magnitude > UINT64_MAX - b.magnitude)
      return false;
...
static void bounds_relax_difference_edge(ixs_bounds *b,
                                         ixs_difference_constraint *edge,
                                         size_t *queue, size_t *tail) {
  ixs_var_bound *lhs_var = &b->vars[edge->lhs_var];
  ixs_var_bound *rhs_var = &b->vars[edge->rhs_var];
  ixs_interval lhs_iv, rhs_iv;
  int64_t endpoint, derived;

  rhs_iv = bounds_get_difference_symbol(b, rhs_var, edge->rhs);
  if (rhs_iv.valid && !rhs_iv.hi_inf) {
    endpoint = ixs_rat_floor(rhs_iv.hi_p, rhs_iv.hi_q);
    if (ixs_safe_add(endpoint, edge->offset, &derived) &&
        bounds_refine_var_upper(b, lhs_var, derived) &&
        *tail < DIFFERENCE_RELAX_LIMIT)
      queue[(*tail)++] = edge->lhs_var;
  }

  lhs_iv = bounds_get_difference_symbol(b, lhs_var, edge->lhs);
  if (lhs_iv.valid && !lhs_iv.lo_inf) {
    endpoint = ixs_rat_ceil(lhs_iv.lo_p, lhs_iv.lo_q);
    if (ixs_safe_sub(endpoint, edge->offset, &derived) &&
        bounds_refine_var_lower(b, rhs_var, derived) &&
        *tail < DIFFERENCE_RELAX_LIMIT)
      queue[(*tail)++] = edge->rhs_var;
  }
}

static void bounds_propagate_difference_bounds(ixs_bounds *b, const char *first,
                                               const char *second) {
  size_t queue[DIFFERENCE_RELAX_LIMIT];
  ixs_var_bound *first_var;
  ixs_var_bound *second_var;
  size_t head = 0;
  size_t tail = 0;
  size_t visited = 0;
  ...
  if (!first_var)
    return;
  queue[tail++] = (size_t)(first_var - b->vars);
  second_var = second && second != first ? find_var(b, second) : NULL;
  if (second_var)
    queue[tail++] = (size_t)(second_var - b->vars);

  /* The worklist follows only incident edges and caps both queued variables
   * and edge visits, independent of the total bounds state. */
  while (head < tail && visited < DIFFERENCE_RELAX_LIMIT && !b->oom &&
         !b->contradiction) {
    size_t var_index = queue[head++];
    ixs_difference_constraint *edge = b->vars[var_index].difference_edges;
    while (edge && visited < DIFFERENCE_RELAX_LIMIT) {
      ixs_difference_constraint *next;
      if (edge->lhs_var == var_index)
        next = edge->next_lhs;
      else if (edge->rhs_var == var_index)
        next = edge->next_rhs;
      else
        break;
      visited++;
      bounds_relax_difference_edge(b, edge, queue, &tail);
      edge = next;
    }
  }
}
...
static void bounds_add_difference_constraint(ixs_bounds *b, ixs_node *lhs,
  ...
  lhs_var = get_or_create_var(b, lhs->u.name);
  rhs_var = get_or_create_var(b, rhs->u.name);
  if (!lhs_var || !rhs_var || b->ndifferences == SIZE_MAX ||
  ...
  edge->lhs = lhs;
  edge->rhs = rhs;
  edge->next_lhs = lhs_var->difference_edges;
  edge->next_rhs = rhs_var->difference_edges;
  edge->lhs_var = (size_t)(lhs_var - b->vars);
  edge->rhs_var = (size_t)(rhs_var - b->vars);
  edge->offset = offset;
  lhs_var->difference_edges = edge;
  rhs_var->difference_edges = edge;
  ...
  bounds_mark_semantic_changed(b);
  bounds_cache_clear(b);
  bounds_propagate_difference_bounds(b, lhs->u.name, rhs->u.name);
}
...
static void bounds_add_difference_range(ixs_bounds *b, ixs_node *expr,
  ...
  ixs_node *lhs;
  ixs_node *rhs;
  int64_t constant, endpoint, offset;
...
static void bounds_add_assumption_impl(ixs_bounds *b, ixs_node *a) {
  ...
  if (op == IXS_CMP_EQ) {
    ...
    else if (extract_cmp_node_equality(a, &equality_lhs, &equality_rhs))
...
static ixs_check_result bounds_project_equality_integer(ixs_bounds *b,
                                                        ixs_node *expr) {
  bounds_equality_walk_entry entries[EQUALITY_WALK_LIMIT + 1u];
  size_t count = 0;
  size_t i;
  ...
  if (!b->nequalities)
    return result;
  status = bounds_collect_equality_component(b, expr, entries, &count);
  ...
  for (i = 0; i < count; i++) {
    ixs_check_result current_result =
        bounds_check_integer_valued_without_equality(b, entries[i].node);
...
static bool bounds_equality_walk_record(bounds_equality_walk_entry *entries,
                                        size_t *count, ixs_node *expr,
                                        bounds_wide_offset offset,
                                        bool *inserted) {
  size_t i;
  for (i = 0; i < *count; i++) {
    if (entries[i].node == expr) {
      *inserted = false;
      return bounds_wide_offset_equal(entries[i].offset, offset);
    }
  }
  if (*count >= EQUALITY_WALK_LIMIT + 1u)
    return false;
  entries[*count].node = expr;
...
/* Collect only the exact-relation component incident to expr.  Entries retain
 * node - expr, and every traversed edge is guarded by independent structural
 * definedness for the newly reached endpoint.  Count unique edges rather than
 * their two adjacency incidences so a 192-edge relation chain stays within
 * the documented 256-edge budget. */
static bounds_equality_walk_status
bounds_collect_equality_component(ixs_bounds *b, ixs_node *expr,
                                  bounds_equality_walk_entry *entries,
                                  size_t *count) {
  ...
  bool inserted;
  bounds_wide_offset zero = {0, false};

  *count = 0;
  if (!b || !expr || !entries || !b->nequalities ||
  ...
  if (!bounds_equality_walk_record(entries, count, expr, zero, &inserted))
    return BOUNDS_EQUALITY_WALK_INVALID;

  while (head < *count) {
    bounds_equality_walk_entry current = entries[head++];
    size_t endpoint_index;
    ixs_equality_edge *edge;
    if (!bounds_find_equality_endpoint(b, current.node, &endpoint_index))
      continue;
    ...
    while (edge) {
      ...
      if (visited_edges++ >= EQUALITY_WALK_LIMIT * 2u ||
          !bounds_equality_edge_neighbor(endpoint_index, edge, &neighbor, &step,
                                         &next))
        return BOUNDS_EQUALITY_WALK_INVALID;
      if (endpoint_index == edge->lhs_endpoint &&
          ++edge_count > EQUALITY_WALK_LIMIT)
        return BOUNDS_EQUALITY_WALK_INVALID;
      edge = next;
      ...
      if (!bounds_wide_offset_add(current.offset, step, &neighbor_offset) ||
          !bounds_equality_walk_record(entries, count, neighbor,
                                       neighbor_offset, &inserted))
        return BOUNDS_EQUALITY_WALK_INVALID;
    }
  }
...
bounds_exact_relation_difference(ixs_bounds *b, ixs_node *lhs, ixs_node *rhs,
  ...
  status = bounds_collect_equality_component(b, rhs, entries, &count);
  if (status != BOUNDS_EQUALITY_WALK_VALID)
    return status;
  for (i = 0; i < count; i++) {
    if (entries[i].node != lhs)
      continue;
    if (!bounds_wide_offset_to_int64(entries[i].offset, delta))
      return BOUNDS_EQUALITY_WALK_INVALID;
    return BOUNDS_EQUALITY_WALK_VALID;
  }
  return BOUNDS_EQUALITY_WALK_NONE;
...
static ixs_interval bounds_project_equality_range(ixs_bounds *b, ixs_node *expr,
  ...
  for (i = 0; i < count; i++) {
    ixs_interval peer;
    ixs_interval shifted;
    ixs_interval merged;
    int64_t delta;
    int64_t negative_delta;
    if (!bounds_wide_offset_to_int64(entries[i].offset, &delta) ||
        !ixs_safe_neg(delta, &negative_delta))
      continue;
    peer = bounds_get_without_equality(b, entries[i].node);
    if (!peer.valid)
      continue;
    shifted = iv_add(peer, ixs_interval_exact(negative_delta, 1));
    if (!shifted.valid)
      continue;
    merged = result.valid ? iv_intersect(result, shifted) : shifted;
...
static int bounds_wide_offset_compare(bounds_wide_offset lhs,
                                      bounds_wide_offset rhs) {
...
static bool bounds_wide_offset_difference(int64_t lhs, int64_t rhs,
                                          bounds_wide_offset *result) {
  return bounds_wide_offset_add(
      bounds_wide_offset_from_int64(lhs),
      bounds_wide_offset_negate(bounds_wide_offset_from_int64(rhs)), result);
}

static uint64_t bounds_wide_offset_residue(bounds_wide_offset value,
                                           uint64_t modulus) {
...
/* Mod preserves its integer dividend modulo the divisor. Literal divisors use
 * a difference enclosure; shared dynamic divisors use a no-wrap proof. The
 * surrounding ADD constant is handled as part of the residual. */
static bool bounds_match_modular_delta_term(ixs_node *expr,
                                            ixs_node **representative,
                                            ixs_node **denominator) {
  if (!expr || expr->tag != IXS_MOD)
    return false;
  *representative = expr->u.binary.lhs;
  *denominator = expr->u.binary.rhs;
  return true;
}

/* The independent value intervals enclose every possible lhs-rhs value. The
 * modular identities put all of them in one residue class. Accept only when
 * that class has exactly one member in the enclosure. */
static bool bounds_unique_modular_delta(bounds_delta_state *state,
                                        ixs_node *lhs, ixs_node *rhs,
                                        int64_t representative_delta,
                                        int64_t modulus, int64_t *delta) {
  bounds_wide_offset lower;
  bounds_wide_offset upper;
  bounds_wide_offset candidate;
  bounds_wide_offset next;
  bounds_wide_offset shift;
  ...
  if (!ixs_bounds_get_integer_range(state->bounds, lhs, &lhs_range) ||
      ...
      !lhs_range.has_lower || !lhs_range.has_upper || !rhs_range.has_lower ||
      !rhs_range.has_upper ||
      !bounds_wide_offset_difference(lhs_range.lower, rhs_range.upper,
                                     &lower) ||
      !bounds_wide_offset_difference(lhs_range.upper, rhs_range.lower,
                                     &upper) ||
      bounds_wide_offset_compare(lower, upper) > 0)
    return false;

  expected_residue =
      (uint64_t)integer_congruence_residue(representative_delta, modulus);
  current_residue = bounds_wide_offset_residue(lower, modulus_u);
  ...
  if (!bounds_wide_offset_add(lower, shift, &candidate) ||
      bounds_wide_offset_compare(candidate, upper) > 0 ||
      !bounds_wide_offset_to_int64(candidate, delta))
    return false;

  shift.magnitude = modulus_u;
  if (bounds_wide_offset_add(candidate, shift, &next) &&
      bounds_wide_offset_compare(next, upper) <= 0)
    return false;
  return true;
}

static ixs_node *bounds_modular_delta_residual(bounds_delta_state *state,
                                               ixs_node *difference,
                                               uint32_t lhs_index,
                                               uint32_t rhs_index) {
  ixs_addterm *terms;
  ...
  count = difference->u.add.nterms - 2u;
  if (count == 0)
    return difference->u.add.coeff;
  terms = ixs_arena_alloc(&state->ctx->scratch, count * sizeof(*terms),
                          sizeof(void *));
  if (!terms) {
    state->oom = true;
    return NULL;
  ...
  if (count == 1u) {
    ixs_node *scaled = simp_mul(state->ctx, terms[0].coeff, terms[0].term);
    if (!scaled) {
      state->oom = true;
      return NULL;
    }
    ...
    if (ixs_node_is_zero(difference->u.add.coeff))
      return scaled;
    scaled = simp_add(state->ctx, difference->u.add.coeff, scaled);
    if (!scaled)
      state->oom = true;
    return scaled && !ixs_node_is_sentinel(scaled) ? scaled : NULL;
  }
  {
    ixs_node *result =
        ixs_node_add(state->ctx, difference->u.add.coeff, count, terms);
    if (!result)
      state->oom = true;
    return result;
  }
...
static bool bounds_constant_delta_core(bounds_delta_state *state, ixs_node *lhs,
                                       ixs_node *rhs, unsigned depth,
                                       int64_t *delta);

static bool bounds_delta_stopped(const bounds_delta_state *state) {
  return state->oom || state->limited || state->invalid;
}
...
/* A stride class shared by the dividend and positive divisor excludes a wrap
 * when the requested shift stays inside one class bucket. This path allocates
 * nothing and rejects before constructing a residual expression. */
...
static bool bounds_collect_modular_delta_terms(
    bounds_delta_state *state, ixs_node *difference, uint32_t *positive,
    size_t *npositive, uint32_t *negative, size_t *nnegative) {
  uint32_t i;
  ...
  *nnegative = 0;
  for (i = 0; i < difference->u.add.nterms; i++) {
    ...
    if (!bounds_match_modular_delta_term(difference->u.add.terms[i].term,
    ...
    if (*count >= MODULAR_DELTA_TERM_LIMIT) {
      state->limited = true;
      return false;
    }
    indices[(*count)++] = i;
...
static bool bounds_modular_delta_pair_valid(bounds_delta_state *state,
                                            ixs_node *lhs_term,
                                            ixs_node *rhs_term,
                                            ixs_node *lhs_representative,
                                            ixs_node *rhs_representative) {
  return ixs_bounds_check_defined(state->bounds, lhs_term) == IXS_CHECK_TRUE &&
         ixs_bounds_check_defined(state->bounds, rhs_term) == IXS_CHECK_TRUE &&
         ixs_bounds_check_defined(state->bounds, lhs_representative) ==
             IXS_CHECK_TRUE &&
         ixs_bounds_check_defined(state->bounds, rhs_representative) ==
             IXS_CHECK_TRUE &&
         ixs_bounds_check_integer_valued(state->bounds, lhs_term) ==
             IXS_CHECK_TRUE &&
         ixs_bounds_check_integer_valued(state->bounds, rhs_term) ==
             IXS_CHECK_TRUE &&
         ixs_bounds_check_integer_valued(state->bounds, lhs_representative) ==
             IXS_CHECK_TRUE &&
         ixs_bounds_check_integer_valued(state->bounds, rhs_representative) ==
             IXS_CHECK_TRUE;
}

static bool bounds_try_modular_delta_pair(bounds_delta_state *state,
                                          ixs_node *difference,
                                          uint32_t lhs_index,
                                          uint32_t rhs_index, unsigned depth,
                                          int64_t *delta) {
  ixs_node *lhs_term = difference->u.add.terms[lhs_index].term;
  ixs_node *rhs_term = difference->u.add.terms[rhs_index].term;
  ixs_node *lhs_representative;
  ixs_node *rhs_representative;
  ...
  int64_t scaled_modular_delta;
  int64_t residual_delta;
  int64_t lhs_coefficient;
  int64_t lhs_coefficient_q;
  int64_t rhs_coefficient;
  int64_t rhs_coefficient_q;
  int64_t opposite_rhs_coefficient;

  ixs_node_get_rat(difference->u.add.terms[lhs_index].coeff, &lhs_coefficient,
                   &lhs_coefficient_q);
  ixs_node_get_rat(difference->u.add.terms[rhs_index].coeff, &rhs_coefficient,
                   &rhs_coefficient_q);

  if (lhs_coefficient_q != 1 || rhs_coefficient_q != 1 ||
      lhs_coefficient <= 0 || rhs_coefficient >= 0 ||
      !ixs_safe_neg(rhs_coefficient, &opposite_rhs_coefficient) ||
      lhs_coefficient != opposite_rhs_coefficient ||
      !bounds_match_modular_delta_term(lhs_term, &lhs_representative,
                                       &lhs_denominator) ||
      !bounds_match_modular_delta_term(rhs_term, &rhs_representative,
                                       &rhs_denominator) ||
      lhs_denominator != rhs_denominator ||
      !bounds_modular_delta_pair_valid(state, lhs_term, rhs_term,
                                       lhs_representative, rhs_representative))
    return false;
  if (!bounds_constant_delta_core(state, lhs_representative, rhs_representative,
                                  depth + 1u, &representative_delta))
    return false;
  if (lhs_denominator->tag == IXS_INT) {
    if (lhs_denominator->u.ival <= 1)
      return false;
    if (!bounds_unique_modular_delta(state, lhs_term, rhs_term,
                                     representative_delta,
                                     lhs_denominator->u.ival, &modular_delta))
      return false;
  } else {
    if (!bounds_denominator_proven_positive(state->bounds, rhs_denominator) ||
        !bounds_mod_shift_by_congruence(state->bounds, rhs_representative,
                                        rhs_denominator, representative_delta))
      return false;
    modular_delta = representative_delta;
  }
  if (!ixs_safe_mul(modular_delta, lhs_coefficient, &scaled_modular_delta))
    return false;
  residual =
      bounds_modular_delta_residual(state, difference, lhs_index, rhs_index);
  if (!residual ||
      !bounds_constant_delta_core(state, residual, state->ctx->node_zero,
                                  depth + 1u, &residual_delta))
    return false;
  return ixs_safe_add(scaled_modular_delta, residual_delta, delta);
}

static bool bounds_modular_delta_expression(bounds_delta_state *state,
                                            ixs_node *difference,
                                            unsigned depth, int64_t *delta) {
  uint32_t positive[MODULAR_DELTA_TERM_LIMIT];
  uint32_t negative[MODULAR_DELTA_TERM_LIMIT];
  size_t npositive = 0;
  size_t nnegative = 0;
  size_t p;
  size_t n;
  ...
  if (!difference || difference->tag != IXS_ADD ||
      difference->u.add.nterms < 2u)
    return false;
  if ((size_t)difference->u.add.nterms > MODULAR_DELTA_VISIT_LIMIT ||
      state->visited >
          MODULAR_DELTA_VISIT_LIMIT - (size_t)difference->u.add.nterms) {
    state->limited = true;
    return false;
  }
  state->visited += difference->u.add.nterms;
  if (!bounds_collect_modular_delta_terms(state, difference, positive,
                                          &npositive, negative, &nnegative))
    return false;
  for (p = 0; p < npositive; p++) {
    for (n = 0; n < nnegative; n++) {
      if (bounds_try_modular_delta_pair(state, difference, positive[p],
                                        negative[n], depth, delta))
        return true;
      if (bounds_delta_stopped(state))
        return false;
    }
  }
  return false;
...
static bool bounds_delta_enter(bounds_delta_state *state, ixs_node *lhs,
                               ixs_node *rhs, unsigned depth) {
  if (!state || !lhs || !rhs || bounds_delta_stopped(state))
    return false;
  if (depth >= MODULAR_DELTA_DEPTH_LIMIT ||
      state->visited >= MODULAR_DELTA_VISIT_LIMIT) {
    state->limited = true;
    return false;
  }
  state->visited++;
  return ixs_bounds_check_defined(state->bounds, lhs) == IXS_CHECK_TRUE &&
         ixs_bounds_check_defined(state->bounds, rhs) == IXS_CHECK_TRUE;
}

static ixs_node *bounds_delta_simplify(bounds_delta_state *state,
                                       ixs_node *expr) {
  if (!expr) {
    state->oom = true;
    return NULL;
  ...
  if (ixs_node_is_sentinel(expr))
    return NULL;
  expr = simp_simplify_bounds(state->ctx, expr, state->bounds);
  if (!expr) {
    state->oom = true;
    return NULL;
  }
  return ixs_node_is_sentinel(expr) ? NULL : expr;
}

static bool bounds_delta_proves_expression(bounds_delta_state *state,
                                           ixs_node *expr, unsigned depth,
                                           int64_t *delta) {
  return bounds_exact_integer_difference(state->bounds, expr, delta) ||
         bounds_modular_delta_expression(state, expr, depth, delta);
}

/* Normalize only inside the proof query. Each recursive projection removes a
 * Mod pair, so depth and work stay bounded by fixed limits. */
static bool bounds_constant_delta_core(bounds_delta_state *state, ixs_node *lhs,
                                       ixs_node *rhs, unsigned depth,
                                       int64_t *delta) {
  ...
  if (!delta || !bounds_delta_enter(state, lhs, rhs, depth))
    return false;
  relation_status =
      bounds_exact_relation_difference(state->bounds, lhs, rhs, delta);
  ...
  difference = bounds_delta_simplify(state, simp_sub(state->ctx, lhs, rhs));
  if (!difference)
    return false;
  if (bounds_delta_proves_expression(state, difference, depth, delta))
    return true;
  if (bounds_delta_stopped(state) || !state->allow_expand)
    return false;

  expanded = bounds_delta_simplify(state, expand_impl(state->ctx, difference));
  if (!expanded)
    return false;
  return expanded != difference &&
         bounds_delta_proves_expression(state, expanded, depth, delta);
}
...
static bool equivalence_integer_delta_mode(equivalence_state *state,
                                           ixs_node *lhs, ixs_node *rhs,
                                           bool allow_expand, int64_t *delta) {
  bounds_delta_state delta_state;
  bool result;
  delta_state.ctx = state->ctx;
  delta_state.bounds = state->bounds;
  delta_state.visited = 0;
  delta_state.allow_expand = allow_expand;
  delta_state.limited = false;
  delta_state.invalid = false;
  delta_state.oom = false;
  result = bounds_constant_delta_core(&delta_state, lhs, rhs, 0, delta);
  if (state->visited > EQUIVALENCE_VISIT_LIMIT ||
      delta_state.visited > EQUIVALENCE_VISIT_LIMIT - state->visited)
    state->limited = true;
  else
    state->visited += delta_state.visited;
  state->limited = state->limited || delta_state.limited || delta_state.invalid;
  state->oom = state->oom || delta_state.oom;
  return result && !state->limited && !state->oom;
}

static bool equivalence_integer_delta(equivalence_state *state, ixs_node *lhs,
                                      ixs_node *rhs, int64_t *delta) {
  return equivalence_integer_delta_mode(state, lhs, rhs, true, delta);
}
...
static ixs_check_result equivalence_difference(equivalence_state *state,
                                               ixs_node *lhs, ixs_node *rhs) {
  int64_t delta;
  if (!equivalence_integer_delta_mode(state, lhs, rhs, false, &delta))
    return IXS_CHECK_UNKNOWN;
...
#define FINITE_EQUIVALENCE_SYMBOL_LIMIT 64u
#define FINITE_EQUIVALENCE_STACK_LIMIT 1024u
#define FINITE_EQUIVALENCE_VISIT_LIMIT 4096u
...
static bool finite_equivalence_insert_symbol(ixs_node **symbols,
                                             size_t *nsymbols,
                                             ixs_node *symbol) {
  size_t at;
  for (at = 0; at < *nsymbols; at++) {
    int order;
    if (symbols[at] == symbol)
      return true;
    order = strcmp(symbol->u.name, symbols[at]->u.name);
    if (order == 0)
      return true;
    if (order < 0)
      break;
  }
  if (*nsymbols >= FINITE_EQUIVALENCE_SYMBOL_LIMIT)
    return false;
  if (at < *nsymbols)
    memmove(&symbols[at + 1u], &symbols[at],
            (*nsymbols - at) * sizeof(*symbols));
  symbols[at] = symbol;
  (*nsymbols)++;
  return true;
}

/* Walk only the queried expression. The fixed limits make symbol discovery
 * independent of unrelated context state; sorted insertion fixes point order.
 */
static bool finite_equivalence_collect_symbols(ixs_node *root,
                                               ixs_node **symbols,
                                               size_t *nsymbols) {
  defined_depth_frame stack[FINITE_EQUIVALENCE_STACK_LIMIT];
  size_t depth = 0;
  size_t visited = 1;
  uint32_t nchildren;
  ...
  *nsymbols = 0;
  if (!root || !defined_child_count(root, &nchildren))
    return false;
  if (root->tag == IXS_SYM &&
      !finite_equivalence_insert_symbol(symbols, nsymbols, root))
    return false;
  if (nchildren == 0)
    return true;
  stack[depth].node = root;
  ...
  while (depth > 0) {
    ...
    if (visited >= FINITE_EQUIVALENCE_VISIT_LIMIT)
      return false;
    child = defined_child_at(frame->node, frame->next_child++);
    visited++;
    if (!child || !defined_child_count(child, &nchildren))
      return false;
    if (child->tag == IXS_SYM &&
        !finite_equivalence_insert_symbol(symbols, nsymbols, child))
      return false;
    if (nchildren == 0)
      continue;
    if (depth >= FINITE_EQUIVALENCE_STACK_LIMIT)
      return false;
    stack[depth].node = child;
    stack[depth].next_child = 0;
    stack[depth].nchildren = nchildren;
...
static bool finite_equivalence_build_domains(
    ixs_bounds *bounds, ixs_node *const *symbols, size_t nsymbols,
    finite_equivalence_domain *domains, size_t *ndomains, size_t *point_count) {
  size_t i;
  *ndomains = 0;
  *point_count = 1;
  for (i = 0; i < nsymbols; i++) {
    ixs_interval interval = ixs_bounds_get(bounds, symbols[i]);
    int64_t lower;
    int64_t upper;
    uint64_t span;
    size_t width;
    if (!interval.valid || interval.lo_inf || interval.hi_inf)
      continue;
    lower = ixs_rat_ceil(interval.lo_p, interval.lo_q);
    upper = ixs_rat_floor(interval.hi_p, interval.hi_q);
    if (upper < lower)
      return false;
    span = (uint64_t)upper - (uint64_t)lower;
    if (span >= (uint64_t)SIZE_MAX)
      return false;
    width = (size_t)span + 1u;
    if (*point_count > SIZE_MAX / width)
      return false;
    *point_count *= width;
    domains[*ndomains].symbol = symbols[i];
    domains[*ndomains].lower = lower;
    domains[*ndomains].upper = upper;
    (*ndomains)++;
  }
  return !bounds->oom;
}
...
static bool
finite_equivalence_prove_point(ixs_ctx *ctx, ixs_bounds *bounds,
                               ixs_node *difference,
                               const finite_equivalence_domain *domains,
                               size_t ndomains, const int64_t *points) {
  ixs_node *targets[FINITE_EQUIVALENCE_SYMBOL_LIMIT];
  ixs_node *replacements[FINITE_EQUIVALENCE_SYMBOL_LIMIT];
  ixs_node *specialized;
  equivalence_state state;
  size_t i;

  for (i = 0; i < ndomains; i++) {
    targets[i] = domains[i].symbol;
    replacements[i] = ixs_node_int(ctx, points[i]);
    if (!replacements[i])
      return false;
  }
  specialized = simp_subs_multi(ctx, difference, (uint32_t)ndomains, targets,
                                replacements);
  if (!specialized || ixs_node_is_sentinel(specialized) ||
      ixs_bounds_check_defined(bounds, specialized) != IXS_CHECK_TRUE)
    return false;
  state.ctx = ctx;
  state.bounds = bounds;
  state.visited = 0;
  state.limited = false;
  state.oom = false;
  if (equivalence_core(&state, specialized, ctx->node_zero, 0) !=
      IXS_CHECK_TRUE)
    return false;
  return !state.oom && !state.limited && !bounds->oom;
}

static void
finite_equivalence_advance_point(const finite_equivalence_domain *domains,
                                 size_t ndomains, int64_t *points) {
  size_t i;
  for (i = ndomains; i > 0; i--) {
    size_t current = i - 1u;
    if (points[current] != domains[current].upper) {
      points[current]++;
      return;
    }
    points[current] = domains[current].lower;
  }
}
...
finite_equivalence_query_bound(ixs_facts *facts, ixs_ctx *ctx, ixs_node *lhs,
  ...
  ixs_arena_mark mark = ixs_arena_save(&ctx->scratch);
  ixs_node *symbols[FINITE_EQUIVALENCE_SYMBOL_LIMIT];
  finite_equivalence_domain domains[FINITE_EQUIVALENCE_SYMBOL_LIMIT];
  int64_t points[FINITE_EQUIVALENCE_SYMBOL_LIMIT];
  ixs_node *difference;
  size_t nsymbols;
  size_t ndomains = 0;
  ...
  if (!difference || ixs_node_is_sentinel(difference) ||
      ixs_bounds_check_defined(&facts->bounds, difference) != IXS_CHECK_TRUE)
    goto restore;
  if (!finite_equivalence_collect_symbols(difference, symbols, &nsymbols))
    goto restore;
  if (!finite_equivalence_build_domains(&facts->bounds, symbols, nsymbols,
                                        domains, &ndomains, &point_count))
    goto restore;
  if (ndomains == 0 || point_count > *remaining_points)
    goto restore;
  ...
  *remaining_points -= point_count;
  for (i = 0; i < ndomains; i++)
    points[i] = domains[i].lower;

  for (point = 0; point < point_count; point++) {
    if (!finite_equivalence_prove_point(ctx, &facts->bounds, difference,
                                        domains, ndomains, points))
      goto restore;
    finite_equivalence_advance_point(domains, ndomains, points);
  }
  result = IXS_CHECK_TRUE;
...
ixs_check_result ixs_equivalent_facts(ixs_facts *facts, ixs_node *lhs,
  ...
  if (ixs_bounds_has_empty(&facts->bounds))
    goto cleanup;
  result = equivalence_query_bound(facts, ctx, lhs, rhs);
...
ixs_check_result ixs_equivalent_finite_domain_facts(ixs_facts *facts,
  ...
  if (result == IXS_CHECK_UNKNOWN)
    result =
        finite_equivalence_query_bound(facts, ctx, lhs, rhs, remaining_points);
...
bool ixs_constant_difference_facts(ixs_facts *facts, ixs_node *lhs,
  ...
  state.bounds = &facts->bounds;
  state.visited = 0;
  state.allow_expand = true;
  state.limited = false;
  state.invalid = false;
  state.oom = false;
  ok = bounds_constant_delta_core(&state, lhs, rhs, 0, &result) &&
       !state.limited && !state.invalid && !state.oom;
