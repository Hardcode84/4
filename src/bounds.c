...
#define BOUNDS_QUERY_VISIT_LIMIT 32768u
...
static bool bounds_add_condition_truth(ixs_bounds *b, ixs_node *cond,
                                       bool truth);
...
bounds_query_begin(ixs_bounds *b, bounds_query_kind kind, ixs_node *expr,
  ...
  *cached = NULL;
  ...
  /* One public fact query and every owner-distinct fork it creates share this
   * generation-wide work budget. Exhaustion is an ordinary proof miss: unlike
   * a transport limit, it must not poison sibling queries or emit a resource
   * diagnostic. */
  if (state->visits >= BOUNDS_QUERY_VISIT_LIMIT)
    return BOUNDS_QUERY_ENTER_BUDGET;
...
IXS_STATIC bool ixs_bounds_init(ixs_bounds *b, ixs_arena *scratch) {
  ...
  ixs_arena_init(&b->work_arena, IXS_ARENA_DEFAULT_SIZE);
  b->ctx = NULL;
  b->store_ctx = NULL;
  b->scratch = scratch;
  b->nvars = 0;
...
IXS_STATIC void ixs_bounds_destroy(ixs_bounds *b) {
  ...
  ixs_arena_destroy_transient(&b->query_arena);
  ixs_arena_destroy_transient(&b->work_arena);
  b->query_state = NULL;
  b->query_owner = 0;
  b->equality_projection_cache = NULL;
  b->equality_projection_cache_count = 0;
...
IXS_STATIC bool ixs_bounds_fork(ixs_bounds *dst, const ixs_bounds *src) {
  ...
  ixs_arena_init(&dst->work_arena, IXS_ARENA_DEFAULT_SIZE);
  dst->ctx = src->ctx;
...
failed:
  ...
  ixs_arena_destroy_transient(&dst->work_arena);
  return false;
}
...
static ixs_expr_bound *bounds_find_expr_override(ixs_bounds *b,
                                                 ixs_node *expr) {
  size_t slot;
  ...
  if (!b->expr_index[slot])
    return NULL;
  return &b->exprs[b->expr_index[slot] - 1u];
}

static ixs_interval bounds_get_expr_overrides(ixs_bounds *b, ixs_node *expr) {
  ixs_expr_bound *bound = bounds_find_expr_override(b, expr);
  return bound ? bound->iv : ixs_interval_unknown();
}
...
static void bounds_add_expr_raw(ixs_bounds *b, ixs_node *expr,
  ...
  if (ixs_node_is_const(expr)) {
    ...
    exact = iv_intersect(iv, ixs_interval_exact(p, q));
    if (!exact.valid || ixs_interval_is_empty(exact))
      bounds_mark_contradiction(b);
    return;
  }
...
static void bounds_add_exact_relation(ixs_bounds *b, ixs_node *lhs,
  ...
  if (!b || !lhs || !rhs || b->oom || b->contradiction)
    return;
  if (ixs_node_is_const(lhs) && ixs_node_is_const(rhs)) {
    int64_t lhs_p;
    int64_t lhs_q;
    int64_t rhs_p;
    int64_t rhs_q;
    int64_t difference_p;
    int64_t difference_q;
    ixs_node_get_rat(lhs, &lhs_p, &lhs_q);
    ixs_node_get_rat(rhs, &rhs_p, &rhs_q);
    if (!ixs_rat_sub(lhs_p, lhs_q, rhs_p, rhs_q, &difference_p,
                     &difference_q) ||
        difference_q != 1 || difference_p != offset)
      bounds_mark_contradiction(b);
    return;
  }
  if (lhs == rhs) {
...
static void bounds_refine_mod_inverse_symbol(ixs_bounds *b, ixs_node *symbol) {
  ...
  work_mark = ixs_arena_save(&b->work_arena);
  incident = ixs_arena_alloc(&b->work_arena, incident_count * sizeof(*incident),
                             sizeof(void *));
  if (!incident) {
    ixs_arena_restore(&b->work_arena, work_mark);
    b->oom = true;
    return;
  }
  watcher_link = b->mod_inverse_heads[var_index];
...
/* Simplify an expression fact under the other known facts, without letting
 * its own stored range prove it constant first.  The result is another key
 * for the existing expression-range map, not a separate relation domain. */
static ixs_node *bounds_simplify_expr_without_own_range(ixs_bounds *b,
                                                        ixs_node *expr) {
  ixs_node *canonical;
  ixs_node *simplified;
  ixs_expr_bound *direct;
  ixs_expr_bound *canonical_bound;
  ixs_interval direct_range = ixs_interval_unknown();
  ixs_interval canonical_range = ixs_interval_unknown();
  ixs_arena_mark diag_mark;
  const char **saved_errors;
  size_t saved_nerrors;
  size_t saved_errors_cap;
  ...
  if (!b || !b->ctx || !expr || b->oom || b->contradiction)
    return NULL;
  canonical = bounds_canonical_expr(b, expr);
  if (b->oom)
    return NULL;
  direct = bounds_find_expr_override(b, expr);
  canonical_bound = canonical && canonical != expr
                        ? bounds_find_expr_override(b, canonical)
                        : NULL;
  if (direct) {
    direct_range = direct->iv;
    direct->iv = ixs_interval_unknown();
  }
  if (canonical_bound) {
    canonical_range = canonical_bound->iv;
    canonical_bound->iv = ixs_interval_unknown();
  }
  bounds_cache_clear(b);
  ...
  saved_nerrors = b->ctx->nerrors;
  saved_errors_cap = b->ctx->errors_cap;
  simplified = simp_simplify_bounds_status(b->ctx, expr, b, &limited);
  ixs_arena_restore(&b->ctx->diag, diag_mark);
  b->ctx->errors = saved_errors;
  b->ctx->nerrors = saved_nerrors;
  b->ctx->errors_cap = saved_errors_cap;
  ...
  if (direct)
    direct->iv = direct_range;
  if (canonical_bound)
    canonical_bound->iv = canonical_range;
  bounds_cache_clear(b);
  if (limited || !simplified || ixs_node_is_sentinel(simplified))
    return NULL;
  return simplified;
}

/* An exact canonical remainder on a symbol is precisely a congruence.  Store
 * it in the same domain as an explicit Mod(x, M) == r fact so every divisor
 * and quotient query sees it. */
static void bounds_apply_exact_mod_congruence(ixs_bounds *b, ixs_node *expr,
                                              ixs_interval range) {
  ixs_node *dividend;
  int64_t modulus;
  int64_t residue;
  ...
  if (!interval_exact_int(&range, &residue) || residue < 0 ||
      residue >= modulus)
    return;

  dividend = expr->u.binary.lhs;
  if (dividend->tag == IXS_SYM)
    apply_modrem(b, dividend->u.name, modulus, residue);
}

static void bounds_refine_congruence_from_mod_range(ixs_bounds *b,
                                                    ixs_node *expr) {
  ixs_interval range;
  ixs_node *alias;
  ixs_node *canonical;
  ...
  if (!b || !expr || expr->tag != IXS_MOD ||
    ...
    return;
  modulus = expr->u.binary.rhs->u.ival;
  range = bounds_get_expr_overrides(b, expr);
  if (!interval_exact_int(&range, &residue) || residue < 0 ||
      residue >= modulus)
    return;

  /* A fact-conditioned rewrite is an equal expression, so its range belongs
   * in the same pointer-indexed map.  Suppressing the source override keeps
   * the range from reducing the source directly to its exact value before
   * algebraic simplification can expose the alias. */
  alias = bounds_simplify_expr_without_own_range(b, expr);
  if (b->oom || b->contradiction)
    return;
  if (alias && alias != expr) {
    bounds_add_expr_raw(b, alias, range);
    if (b->oom || b->contradiction)
      return;
    bounds_apply_exact_mod_congruence(b, alias, range);
    canonical = bounds_canonical_expr(b, alias);
    if (canonical && canonical != alias) {
      bounds_add_expr_raw(b, canonical, range);
      if (!b->oom && !b->contradiction)
        bounds_apply_exact_mod_congruence(b, canonical, range);
    }
  }
  if (!b->oom && !b->contradiction)
    bounds_apply_exact_mod_congruence(b, expr, range);
}
...
static bool bounds_finite_interval_endpoint(const ixs_interval *iv, bool lower,
                                            int64_t *p, int64_t *q) {
  int64_t endpoint_p;
  int64_t endpoint_q;
  if (!iv || !iv->valid || (lower ? iv->lo_inf : iv->hi_inf))
    return false;
  endpoint_p = lower ? iv->lo_p : iv->hi_p;
  endpoint_q = lower ? iv->lo_q : iv->hi_q;
...
static bool bounds_collect_add_term_ranges(ixs_bounds *b, ixs_node *expr,
  ...
  *sum = ixs_interval_exact(constant_p, constant_q);
  for (i = 0; i < expr->u.add.nterms; i++) {
    ...
    term_ranges[i] = ixs_bounds_get(b, expr->u.add.terms[i].term);
    ixs_node_get_rat(expr->u.add.terms[i].coeff, &coefficient_p,
                     &coefficient_q);
    if (!term_ranges[i].valid || coefficient_p == 0)
      return false;
    contribution = iv_mul_const(term_ranges[i], coefficient_p, coefficient_q);
...
static bool bounds_select_add_term_endpoints(ixs_node *expr,
                                             ixs_interval *term_ranges,
  ...
  for (i = 0; i < expr->u.add.nterms; i++) {
    ...
    if (!bounds_finite_interval_endpoint(&term_ranges[i], use_lower,
                                         &endpoint_p, &endpoint_q))
      return false;
    term_ranges[i] = ixs_interval_exact(endpoint_p, endpoint_q);
  }
  return true;
}
...
static void bounds_publish_add_term_endpoints(ixs_bounds *b, ixs_node *expr,
  ...
  for (;;) {
    bool *outer_changed = b->semantic_changed;
    bool changed = false;

    b->semantic_changed = &changed;
    for (i = 0; i < expr->u.add.nterms && !b->oom && !b->contradiction; i++)
      ixs_bounds_add_expr(b, expr->u.add.terms[i].term, term_ranges[i]);
    b->semantic_changed = outer_changed;
    if (changed && outer_changed)
      *outer_changed = true;
...
static void bounds_refine_add_from_endpoint(ixs_bounds *b, ixs_node *expr) {
  ...
  if (!b || !expr || expr->tag != IXS_ADD || expr->u.add.nterms == 0 ||
    ...
    return;
  fixed = bounds_get_expr_overrides(b, expr);
  ...
  range_bytes = (size_t)expr->u.add.nterms * sizeof(*term_ranges);
  if (range_bytes / sizeof(*term_ranges) != expr->u.add.nterms) {
    b->oom = true;
    return;
  }
  mark = ixs_arena_save(&b->work_arena);
  term_ranges = ixs_arena_alloc(&b->work_arena, range_bytes, sizeof(void *));
  if (!term_ranges) {
    b->oom = true;
    goto cleanup;
  }
  if (!bounds_collect_add_term_ranges(b, expr, term_ranges, &sum) ||
      !bounds_select_add_term_endpoints(expr, term_ranges, sum, fixed_p,
                                        fixed_q))
    goto cleanup;
  bounds_publish_add_term_endpoints(b, expr, term_ranges);

cleanup:
  ixs_arena_restore(&b->work_arena, mark);
}
...
IXS_STATIC void ixs_bounds_add_expr(ixs_bounds *b, ixs_node *expr,
  ...
  if (!bounds_propagate_added_expr_ranges(b, expr, canon, iv, first_expr,
    ...
    return;
  for (i = first_expr; i < added_expr_end; i++) {
    bounds_refine_congruence_from_mod_range(b, b->exprs[i].expr);
    bounds_refine_mod_inverse_for_expr(b, b->exprs[i].expr);
  }
  bounds_refine_congruence_from_mod_range(b, expr);
  bounds_refine_mod_inverse_for_expr(b, expr);
  if (canon && canon != expr) {
    bounds_refine_congruence_from_mod_range(b, canon);
...
typedef enum {
  ...
  BOUNDS_EXACT_PROOF_DIVISIBLE_ADD_REDUCED,
...
/* Apply the Euclidean quotient/remainder identity inside one canonical ADD:
 *
 *   c*A - c*Mod(A,D) = c*D*floor(A/D),  integer A, positive integer D.
 *
...
static ixs_node *bounds_exact_proof_reduce_mod_pair(ixs_bounds *bounds,
  ...
  if (!ctx || !node || node->tag != IXS_ADD)
    return node;
  for (mod_index = 0; mod_index < node->u.add.nterms; mod_index++) {
    const ixs_addterm *mod_term = &node->u.add.terms[mod_index];
    ixs_node *mod = mod_term->term;
    ixs_node *base;
    int64_t divisor;
    ...
    if (!mod || mod->tag != IXS_MOD || !mod->u.binary.lhs ||
        !mod->u.binary.rhs || mod->u.binary.rhs->tag != IXS_INT ||
        mod->u.binary.rhs->u.ival <= 0)
      continue;
    base = mod->u.binary.lhs;
    divisor = mod->u.binary.rhs->u.ival;
    if (!ixs_node_is_integer_valued(base))
      continue;
    ixs_node_get_rat(mod_term->coeff, &mod_coefficient, &mod_q);
    if (mod_q != 1 || !ixs_safe_neg(mod_coefficient, &opposite))
      continue;
...
bounds_exact_proof_start_divisible_add(bounds_exact_proof_query *query,
  ...
  if (p % frame->modulus != 0)
    return bounds_exact_proof_divisible_after_add(query, frame);
  reduced = bounds_exact_proof_reduce_mod_pair(query->bounds, node);
  if (!reduced)
    return BOUNDS_EXACT_PROOF_STEP_OOM;
  if (reduced != node) {
...
bounds_exact_proof_resume_divisible_add(bounds_exact_proof_query *query,
                                        bounds_exact_proof_frame *frame) {
  ixs_node *node = frame->expr;
  if (frame->stage == BOUNDS_EXACT_PROOF_DIVISIBLE_ADD_REDUCED)
    return bounds_exact_proof_complete(query, query->child_result);
...
bounds_exact_proof_resume(bounds_exact_proof_query *query,
  ...
  case BOUNDS_EXACT_PROOF_DIVISIBLE_ADD_REDUCED:
...
static bool bounds_residue_add_condition(bounds_residue_query *query,
                                         ...
                                         bool truth) {
  if (bounds_add_condition_truth(target, cond, truth))
    return true;
  if (target->oom)
    query->root->oom = true;
  return false;
 ...
 * those literals.  Structural domain proofs keep this interval query from
 * recursively asking for ranges while it is already aggregating an ADD. */
...
static bool bounds_get_add_residue_groups(ixs_bounds *b, ixs_node *expr,
  ...
  for (i = 0; i < capacity && ngroups != 0; i++) {
    bounds_add_residue_group *group = &groups[i];
    ixs_interval interval;
    uint64_t reduced;
...
typedef struct {
  ...
  ixs_node *divisor;
  ixs_node *argument;
} bounds_trunc_remainder;

static bool bounds_try_product(ixs_bounds *b, ixs_node *lhs, ixs_node *rhs,
                               ixs_node **result) {
  bool unrepresentable = false;
  *result = simp_try_mul(b->ctx, lhs, rhs, &unrepresentable);
  if (!*result && !unrepresentable)
    b->oom = true;
  return *result != NULL && !ixs_node_is_sentinel(*result);
}

static bool bounds_try_sum(ixs_bounds *b, ixs_node *lhs, ixs_node *rhs,
                           ixs_node **result) {
  bool unrepresentable = false;
  *result = simp_try_add(b->ctx, lhs, rhs, &unrepresentable);
  if (!*result && !unrepresentable)
    b->oom = true;
  return *result != NULL && !ixs_node_is_sentinel(*result);
}

static bool bounds_build_trunc_remainder(ixs_bounds *b, ixs_node *expr,
                                         uint32_t term_index,
                                         uint32_t factor_index,
                                         bool direct_trunc,
                                         bounds_trunc_remainder *match) {
  const ixs_addterm *candidate;
  ixs_node *trunc;
  ixs_node *negative_divisor;
  ixs_node *divisor;
  ixs_node *dividend;
  uint32_t i;

  if (!b || !b->ctx || !expr || expr->tag != IXS_ADD ||
      term_index >= expr->u.add.nterms)
    return false;
  candidate = &expr->u.add.terms[term_index];
  if (direct_trunc) {
    if (candidate->term->tag != IXS_TRUNC)
      return false;
    trunc = candidate->term;
  } else {
    if (candidate->term->tag != IXS_MUL ||
        factor_index >= candidate->term->u.mul.nfactors ||
        candidate->term->u.mul.factors[factor_index].exp != 1 ||
        candidate->term->u.mul.factors[factor_index].base->tag != IXS_TRUNC)
      return false;
    trunc = candidate->term->u.mul.factors[factor_index].base;
  }

  negative_divisor = candidate->coeff;
  divisor = NULL;
  dividend = expr->u.add.coeff;
  if (!direct_trunc) {
    ixs_node *term = candidate->term;
    if (!bounds_try_product(b, negative_divisor, term->u.mul.coeff,
                            &negative_divisor))
      return false;
    for (i = 0; i < term->u.mul.nfactors; i++) {
      ...
      if (i == factor_index)
        continue;
      factor = term->u.mul.factors[i];
      power = factor.exp == 1
                  ? factor.base
                  : ixs_node_mul(b->ctx, b->ctx->node_one, 1, &factor);
      if (!power) {
        b->oom = true;
        return false;
      }
      if (!bounds_try_product(b, negative_divisor, power, &negative_divisor))
        return false;
    }
  }
  if (!bounds_try_product(b, ixs_node_int(b->ctx, -1), negative_divisor,
                          &divisor))
    return false;
  ...
  for (i = 0; i < expr->u.add.nterms; i++) {
    ...
    if (i == term_index)
      continue;
    if (!bounds_try_product(b, expr->u.add.terms[i].coeff,
                            expr->u.add.terms[i].term, &scaled) ||
        !bounds_try_sum(b, dividend, scaled, &dividend))
      return false;
  }
  match->expr = expr;
  match->dividend = dividend;
  match->divisor = divisor;
  match->argument = trunc->u.unary.arg;
  return true;
}

static bool bounds_trunc_remainder_identity(ixs_bounds *b,
                                            bounds_trunc_remainder *match) {
  ixs_arena_mark diag_mark;
  const char **saved_errors;
  size_t saved_nerrors;
  ...
  if (!bounds_try_product(b, match->divisor, match->argument, &product))
    return false;
  equality = simp_cmp(b->ctx, product, IXS_CMP_EQ, match->dividend);
  if (!equality) {
    b->oom = true;
    return false;
  }
  if (ixs_node_is_sentinel(equality))
    return false;
  ...
  saved_nerrors = b->ctx->nerrors;
  saved_errors_cap = b->ctx->errors_cap;
  simplified = simp_simplify_bounds_status(b->ctx, equality, b, &limited);
  ixs_arena_restore(&b->ctx->diag, diag_mark);
  b->ctx->errors = saved_errors;
  b->ctx->nerrors = saved_nerrors;
  b->ctx->errors_cap = saved_errors_cap;
  ...
  if (!limited && simplified && !ixs_node_is_sentinel(simplified))
    proved = ixs_node_is_known_true(simplified);
  else if (!limited && !simplified)
    b->oom = true;
  return proved;
}

static bool bounds_trunc_remainder_nonzero_range(ixs_bounds *b,
                                                 ixs_node *divisor, int sign,
                                                 ixs_interval *result) {
  ixs_interval divisor_range = ixs_bounds_get(b, divisor);
  int64_t endpoint;
  int64_t radius;

  if (!divisor_range.valid)
    return false;
  if (sign > 0) {
    if (divisor_range.hi_inf)
      return false;
    endpoint = ixs_rat_floor(divisor_range.hi_p, divisor_range.hi_q);
    if (endpoint < 1 || !ixs_safe_sub(endpoint, 1, &radius))
      return false;
  } else {
    if (divisor_range.lo_inf)
      return false;
    endpoint = ixs_rat_ceil(divisor_range.lo_p, divisor_range.lo_q);
    if (endpoint > -1 || !ixs_safe_add(endpoint, 1, &endpoint) ||
        !ixs_safe_neg(endpoint, &radius))
      return false;
  }
  *result = ixs_interval_range(-radius, 1, radius, 1);
  return true;
}

static bool bounds_trunc_remainder_branch(ixs_bounds *owner,
                                          bounds_trunc_remainder *match,
                                          ixs_cmp_op branch_op, int sign,
                                          ixs_interval *result,
                                          bool *reachable) {
  ...
  *reachable = false;
  ...
  condition =
      simp_cmp(owner->ctx, match->divisor, branch_op, owner->ctx->node_zero);
  if (!condition) {
    owner->oom = true;
    goto cleanup;
  }
  if (ixs_node_is_sentinel(condition))
    goto cleanup;
  if (!ixs_bounds_fork(&branch, owner)) {
    owner->oom = true;
    goto cleanup;
  }
  branch_ready = true;
  if (!bounds_add_condition_truth(&branch, condition, true)) {
    if (branch.oom)
      owner->oom = true;
    goto cleanup;
  }
  ...
  *reachable = true;
  if (ixs_bounds_check_defined(&branch, match->expr) != IXS_CHECK_TRUE ||
      ixs_bounds_check_defined(&branch, match->dividend) != IXS_CHECK_TRUE ||
      ixs_bounds_check_integer_valued(&branch, match->dividend) !=
          IXS_CHECK_TRUE ||
      ixs_bounds_check_integer_valued(&branch, match->divisor) !=
          IXS_CHECK_TRUE)
    goto cleanup;

  if (sign == 0) {
    *result = ixs_bounds_get(&branch, match->dividend);
    ok = result->valid;
  } else if (bounds_trunc_remainder_identity(&branch, match)) {
    ok = bounds_trunc_remainder_nonzero_range(&branch, match->divisor, sign,
                                              result);
  }

cleanup:
  if (branch_ready && branch.oom)
    owner->oom = true;
  if (branch_ready)
...
static bool bounds_get_trunc_remainder_match(ixs_bounds *b,
                                             bounds_trunc_remainder *match,
                                             ixs_interval *result) {
  static const ixs_cmp_op branch_ops[] = {IXS_CMP_GT, IXS_CMP_LT, IXS_CMP_EQ};
  static const int branch_signs[] = {1, -1, 0};
  bool have_result = false;
  size_t i;

  for (i = 0; i < sizeof(branch_ops) / sizeof(branch_ops[0]); i++) {
    ixs_interval branch;
    bool reachable;
    if (!bounds_trunc_remainder_branch(b, match, branch_ops[i], branch_signs[i],
                                       &branch, &reachable))
      return false;
    if (!reachable)
      continue;
  ...
  return have_result && result->valid;
}

static bool bounds_get_trunc_remainder(ixs_bounds *b, ixs_node *expr,
                                       ixs_interval *result) {
  uint32_t i;

  if (!b || !expr || expr->tag != IXS_ADD)
    return false;
  for (i = 0; i < expr->u.add.nterms && !b->oom; i++) {
    ixs_node *term = expr->u.add.terms[i].term;
    bounds_trunc_remainder match;
    uint32_t j;

    if (term->tag == IXS_TRUNC &&
        bounds_build_trunc_remainder(b, expr, i, 0, true, &match) &&
        bounds_get_trunc_remainder_match(b, &match, result))
      return true;
    if (term->tag != IXS_MUL)
      continue;
    for (j = 0; j < term->u.mul.nfactors && !b->oom; j++) {
      if (term->u.mul.factors[j].exp != 1 ||
          term->u.mul.factors[j].base->tag != IXS_TRUNC)
        continue;
      if (bounds_build_trunc_remainder(b, expr, i, j, false, &match) &&
          bounds_get_trunc_remainder_match(b, &match, result))
        return true;
    }
  }
...
static inline ixs_interval bounds_get_add(ixs_bounds *b, ixs_node *expr) {
  ...
  for (i = 0; i < expr->u.add.nterms; i++) {
    ixs_node *term = expr->u.add.terms[i].term;
    int64_t cp, cq;
    ixs_interval ti = ixs_bounds_get(b, term);
    ixs_interval scaled;
  ...
  if (!b->oom) {
    ixs_interval grouped;
    if (bounds_get_add_residue_groups(b, expr, residue_candidate_count,
                                      &grouped))
      result = iv_intersect(result, grouped);
  }
  if (!b->oom && b->nexprs != 0)
  ...
  if (!b->oom) {
    ixs_interval remainder;
    if (bounds_get_trunc_remainder(b, expr, &remainder))
      result = iv_intersect(result, remainder);
  }
  return result;
}
...
/* For M = 2^n, a known residue modulo 2^k confines Mod(dividend, M) to
 * r, r + 2^k, ..., r + M - 2^k.  Find the strongest congruence supplied by
 * the generic residue algebra with a bounded logarithmic search. */
static bool bounds_pow2_residue_mod_range(ixs_bounds *b, ixs_node *dividend,
                                          int64_t modulus, ixs_interval *out) {
  unsigned lower = 1u;
  unsigned upper_exp;
  unsigned best = 0u;
  ...
  if (!int64_modulus_is_pow2(modulus) || modulus <= 1)
    return false;
  upper_exp = bit_ctz64((uint64_t)modulus);
  while (lower <= upper_exp) {
    unsigned middle = lower + (upper_exp - lower) / 2u;
    uint64_t candidate = UINT64_C(1) << middle;
    uint64_t candidate_residue;
    if (bounds_known_residue(b, dividend, candidate, &candidate_residue)) {
      best = middle;
...
static ixs_interval bounds_get_positive_mod(ixs_bounds *b, ixs_node *lhs,
  ...
  if (b->has_modrem && !residue_tried && ixs_node_is_integer_valued(lhs) &&
      ...
      bounds_pow2_residue_mod_range(b, lhs, modulus, &congruent))
    return congruent;
...
static inline ixs_interval bounds_get_extrema(ixs_bounds *b, ixs_node *expr,
  ...
  /* A single known operand still gives a sound one-sided extrema bound:
   * Max(known, unknown) is at least known, while Min(known, unknown) is at
   * most known.  Only the opposite side depends on every operand. */
  if (!all_known) {
    if (is_max)
      ixs_interval_set_hi_pos_inf(&result);
    else
...
static bool bounds_condition_truth_work_push(ixs_arena *arena,
  ...
  (*work)[*count].condition = condition;
  (*work)[*count].truth = truth;
  (*count)++;
  return true;
}

/* Publish the deterministic consequences of a Boolean branch.  A true AND
 * and a false OR entail the same truth value for every child; NOT reverses
 * polarity.  The opposite polarities remain a single assumption because
 * choosing any one child there would be unsound. */
static bool bounds_add_condition_truth(ixs_bounds *b, ixs_node *cond,
                                       bool truth) {
  ixs_arena *arena;
  ixs_arena_mark mark;
  bounds_condition_truth_work *work = NULL;
  size_t count = 0;
  size_t capacity = 0;
  bool ok = false;

  if (!b || !b->ctx || !cond || b->oom)
    return false;
  arena = &b->work_arena;
  mark = ixs_arena_save(arena);
  if (!bounds_condition_truth_work_push(arena, &work, &count, &capacity, cond,
                                        truth))
    goto oom;

  while (count != 0u) {
    bounds_condition_truth_work item = work[--count];
    ixs_node *condition = item.condition;
    ...
    if ((item.truth && condition->tag == IXS_AND) ||
        (!item.truth && condition->tag == IXS_OR)) {
...
static bool bounds_piecewise_active(ixs_bounds *owner, ixs_bounds *remaining,
  ...
  active_ready = true;
  if (!bounds_add_condition_truth(&active, cond, true)) {
    if (active.oom)
      owner->oom = true;
    goto cleanup;
  }
...
static ixs_interval bounds_get_piecewise(ixs_bounds *b, ixs_node *expr) {
  ...
  for (i = 0; i < expr->u.pw.ncases; i++) {
    ...
    if (!bounds_add_condition_truth(&remaining, cond, false)) {
      b->oom = true;
      failed = true;
      break;
    }
...
/* A comparison proving a supported boolean predicate exactly true entails
 * that predicate. Comparisons are normally represented as ADD(k, +/-expr) OP
 * 0, so use the shared normalized extractor instead of matching surface
 * syntax. Keep unsupported boolean operators opaque: direct OR and NOT facts
 * remain outside the assumption-ingestion contract. */
static ixs_node *assumption_exact_true_predicate(ixs_node *predicate) {
  ixs_node *child;
  int64_t value;
  if (!predicate || predicate->tag != IXS_CMP ||
      predicate->u.binary.cmp_op != IXS_CMP_EQ ||
      !extract_cmp_expr_const(predicate, &child, &value) || value != 1 ||
      !ixs_node_is_bool_valued(child) ||
      (child->tag != IXS_CMP && child->tag != IXS_AND))
    return NULL;
  return child;
}
...
static ixs_bounds_build_status bounds_start_predicate_frame(
  ...
  if (cur == b->ctx->node_false) {
    ...
    if (status != IXS_BOUNDS_BUILD_OK)
      return status;
    frame->nchildren = 0u;
    /* Canonicalize an exact Boolean truth wrapper to the supported predicate
     * it proves. Keeping only the child avoids duplicate closure work and
     * prevents an outer identity from hiding the comparison or conjunction. */
    frame->truth_child = assumption_exact_true_predicate(cur);
    if (frame->truth_child) {
      frame->nchildren = 1u;
      return IXS_BOUNDS_BUILD_OK;
    }
...
bounds_process_predicate(ixs_bounds *b, ixs_node *pred, bool ingest) {
  ...
  while (depth > 0) {
    ...
    if (frame->next_child < frame->nchildren) {
      ixs_node *child = frame->truth_child
                            ? frame->truth_child
                            : cur->u.assoc.args[frame->next_child];
      frame->next_child++;
      entry =
...
static void facts_commit(ixs_facts *facts, ixs_bounds *candidate) {
  ...
  ixs_arena_destroy_transient(&facts->bounds.work_arena);
...
static bool query_node_set_contains(const query_node_set *set,
                                    const ixs_node *node) {
  size_t index;
  if (!set || !node || !set->capacity)
    return false;
  index = node->hash & (set->capacity - 1u);
  while (set->slots[index] && set->slots[index] != node)
    index = (index + 1u) & (set->capacity - 1u);
  return set->slots[index] == node;
}
...
/* Keep a supported predicate proved true as the closure root instead of
 * repeatedly simplifying only its outer `predicate == 1` fact. Once the outer
 * fact is stored it can prove itself true and hide consequences enabled by a
 * later sibling. The predicate root cannot hide those consequences:
 * re-simplify and ingest it until one pass makes no semantic refinement.
 * Bounds refinement is monotone, so termination depends only on reaching the
 * actual fixed point, never on a round limit. */
static ixs_bounds_build_status
facts_saturate_true_predicate(ixs_ctx *ctx, ixs_bounds *candidate,
                              ixs_node *predicate) {
  ixs_node *root = assumption_exact_true_predicate(predicate);
  bool *outer_changed = candidate->semantic_changed;
  ...
  for (;;) {
    ...
    candidate->semantic_changed = outer_changed;
    if (changed && outer_changed)
      *outer_changed = true;
...
facts_process_predicate_worklist(ixs_ctx *ctx, ixs_bounds *candidate,
  ...
  while (work->queue_count > 0) {
    ...
    if (status == IXS_BOUNDS_BUILD_OK)
      status = facts_saturate_true_predicate(ctx, candidate, predicate);
    candidate->semantic_changed = NULL;
    if (status != IXS_BOUNDS_BUILD_OK)
      return status;
...
typedef enum {
  FACTS_DEFINITION_OK,
  FACTS_DEFINITION_LIMITED,
  FACTS_DEFINITION_INVALID,
  FACTS_DEFINITION_OOM
} facts_definition_status;

typedef struct {
  query_node_set targets;
  ixs_node **ordered;
  size_t count;
  size_t capacity;
...
facts_definition_collect_targets(ixs_ctx *ctx, ixs_bounds *bounds,
  ...
  while (stack_count > 0) {
    ...
    if (!inserted)
      continue;
    if (node->tag == IXS_SYM &&
        bounds_find_equality_endpoint(bounds, node, &endpoint_index)) {
      (void)endpoint_index;
      if (!query_node_set_insert(&ctx->scratch, &targets->targets, node,
                                 &inserted))
        return FACTS_DEFINITION_OOM;
      if (inserted &&
    ...
    for (i = 0; i < child_count; i++) {
      ...
      if (!child)
        return FACTS_DEFINITION_INVALID;
      if (!query_node_stack_push(&ctx->scratch, &stack, &stack_count,
                                 &stack_capacity, child))
        return FACTS_DEFINITION_OOM;
    }
  ...
  *intersects = false;
  ...
  while (stack_count > 0) {
    ...
    for (i = 0; i < child_count; i++) {
      ixs_node *child = ixs_node_child(node, i);
      if (!child) {
        status = FACTS_DEFINITION_INVALID;
        goto cleanup;
      }
      if (!query_node_stack_push(&ctx->scratch, &stack, &stack_count,
                                 &stack_capacity, child)) {
...
facts_definition_replacement(ixs_ctx *ctx, ixs_bounds *bounds, ixs_node *target,
  ...
  *replacement = NULL;
  walk_status = bounds_collect_equality_component(bounds, target, &walk, true);
  if (walk_status == BOUNDS_EQUALITY_WALK_NONE)
    return FACTS_DEFINITION_OK;
  if (walk_status != BOUNDS_EQUALITY_WALK_VALID) {
    bounds_equality_walk_destroy(bounds, &walk);
    if (walk_status == BOUNDS_EQUALITY_WALK_LIMITED)
      return FACTS_DEFINITION_LIMITED;
    if (walk_status == BOUNDS_EQUALITY_WALK_OOM)
      return FACTS_DEFINITION_OOM;
    return FACTS_DEFINITION_INVALID;
  }

  for (i = 0; i < walk.count; i++) {
    ixs_node *candidate = walk.entries[i].node;
    bounds_wide_offset delta;
    int64_t delta_value;
    bool intersects;
    facts_definition_status status;

    if (candidate == target || query_node_set_contains(targets, candidate))
      continue;
    status = facts_definition_intersects_targets(ctx, candidate, targets,
                                                 &intersects);
    ...
    if (intersects)
      continue;
    delta = bounds_wide_offset_negate(walk.entries[i].offset);
    if (!bounds_wide_offset_to_int64(delta, &delta_value))
      continue;
    if (delta_value != 0) {
      ...
      candidate = simp_try_add(ctx, candidate, constant, &unrepresentable);
      if (!candidate) {
        if (unrepresentable)
          continue;
        bounds_equality_walk_destroy(bounds, &walk);
...
/* Equality definitions are selected from the original query and applied in
 * one substitution pass. Replacements are never traversed, and no replacement
...
facts_apply_equality_definitions(ixs_ctx *ctx, ixs_bounds *bounds,
  ...
  *result = expr;
  if (!bounds->nequalities)
    return FACTS_DEFINITION_OK;
  status = facts_definition_collect_targets(ctx, bounds, expr, &definitions);
  if (status != FACTS_DEFINITION_OK || definitions.count == 0)
    return status;
  ...
  for (i = 0; i < definitions.count; i++) {
    ...
    if (!replacement)
      continue;
    definitions.ordered[replacement_count] = definitions.ordered[i];
    replacements[replacement_count++] = replacement;
  }
  if (replacement_count == 0)
    return FACTS_DEFINITION_OK;
  *result = simp_subs_multi(ctx, expr, (uint32_t)replacement_count,
                            definitions.ordered, replacements);
  if (!*result)
    return FACTS_DEFINITION_OOM;
...
facts_query_simplify_batch(ixs_facts *facts, ixs_node **exprs, size_t n) {
  ...
  facts_definition_status definition_status = FACTS_DEFINITION_OK;
  bool ok;
  bool old_oom;
  bool query_held = false;
...
static ixs_check_result predicate_query_cmp_atom(ixs_bounds *bounds,
  ...
  predicate_fact = bounds_get_expr_overrides(bounds, node);
  if (ixs_interval_is_point_int(predicate_fact, &exact) &&
      (exact == 0 || exact == 1))
    return exact == 0 ? IXS_CHECK_FALSE : IXS_CHECK_TRUE;
...
/* Context-aware Boolean complement subsumption:
 *
 *   P | ~(P & R) = 1
 *   P & ~(P | R) = 0
 *
 * AND/OR are eager, so the identity is usable only after facts prove the
 * complete source predicate defined and integer-valued.  Pointer membership
 * is sufficient because associative nodes contain canonical interned nodes. */
static ixs_check_result
predicate_query_complement_subsumption(ixs_bounds *bounds,
                                       ixs_node *predicate) {
  ixs_tag nested_tag;
  uint32_t i;

  if (!bounds || !predicate ||
      (predicate->tag != IXS_AND && predicate->tag != IXS_OR) ||
      !predicate_query_assoc_domain_proven(bounds, predicate))
    return IXS_CHECK_UNKNOWN;
  nested_tag = predicate->tag == IXS_OR ? IXS_AND : IXS_OR;
  for (i = 0u; i < predicate->u.assoc.nargs; i++) {
    ixs_node *negated = predicate->u.assoc.args[i];
    ixs_node *nested;
    uint32_t j;
    ...
    if (nested->tag != nested_tag)
      continue;
    for (j = 0u; j < predicate->u.assoc.nargs; j++) {
      uint32_t k;
      if (j == i)
        continue;
      for (k = 0u; k < nested->u.assoc.nargs; k++) {
        if (predicate->u.assoc.args[j] == nested->u.assoc.args[k])
          return predicate->tag == IXS_OR ? IXS_CHECK_TRUE : IXS_CHECK_FALSE;
      }
    }
  }
  return IXS_CHECK_UNKNOWN;
...
/* Assumption ingestion deliberately accepts only conjunctions and comparison
 * leaves.  A private implication branch may nevertheless need to assume an
 * arbitrary Boolean-valued disjunct true.  Preserve that exact truth as an
...
static ixs_node *predicate_query_true_antecedent(ixs_ctx *ctx,
  ...
  if (!ixs_node_is_bool_valued(predicate))
    return NULL;
  return ixs_node_binary(ctx, IXS_CMP, predicate, ctx->node_true, IXS_CMP_EQ);
}

/* An n-way disjunction is equivalent to
 *
 *   (!d0 & ... & !d(i-1) & !d(i+1) & ... & !dn) => di.
 *
 * Try each disjunct as the consequent when every other disjunct has an
 * ingestible complement.  This retains the complete conjunction as one
 * closure root, so nested guarded goals do not lose either the relation domain
 * or the inner guard when associative OR construction flattens them.  Explicit
 * NOT nodes contribute their child directly; comparisons have already
 * absorbed NOT during canonicalization, so reconstruct their complementary
 * comparison.  The source predicate must remain total: AND/OR are eager
 * integer operators, so a true branch does not hide an undefined sibling. */
static ixs_check_result predicate_query_implication(ixs_bounds *bounds,
                                                    ixs_node *predicate,
                                                    bool *limited) {
  ixs_arena *arena;
  ixs_arena_mark mark;
  ixs_node **antecedents;
  uint32_t consequent_index;
  uint32_t nargs;
  ixs_check_result answer = IXS_CHECK_UNKNOWN;
  ...
  if (!bounds || !predicate || predicate->tag != IXS_OR ||
      !predicate_query_assoc_domain_proven(bounds, predicate))
    return IXS_CHECK_UNKNOWN;
  nargs = predicate->u.assoc.nargs;
  if (nargs < 2u)
    return IXS_CHECK_UNKNOWN;

  arena = &bounds->work_arena;
  mark = ixs_arena_save(arena);
  antecedents = ixs_arena_alloc(
      arena, (size_t)(nargs - 1u) * sizeof(*antecedents), sizeof(void *));
  if (!antecedents) {
    bounds->oom = true;
    goto cleanup;
  }

  for (consequent_index = 0u; consequent_index < nargs; consequent_index++) {
    ixs_node *antecedent;
    uint32_t antecedent_count = 0u;
    uint32_t i;
    bool supported = true;
    bool branch_limited = false;
    ...
    for (i = 0u; i < nargs; i++) {
      ixs_node *disjunct;
      ixs_node *complement;
      if (i == consequent_index)
        continue;
      disjunct = predicate->u.assoc.args[i];
      if (disjunct->tag == IXS_NOT) {
        complement = predicate_query_true_antecedent(
            bounds->ctx, disjunct->u.unary_bool.arg);
        if (!complement) {
          supported = false;
          break;
        }
      } else if (disjunct->tag == IXS_CMP) {
        complement = simp_not(bounds->ctx, disjunct);
        if (!complement) {
          bounds->oom = true;
          goto cleanup;
        }
      ...
      antecedents[antecedent_count++] = complement;
    }
    if (!supported)
      continue;
    antecedent =
        antecedent_count == 1u
            ? antecedents[0]
            : simp_and_many(bounds->ctx, antecedent_count, antecedents);
    if (!antecedent) {
      bounds->oom = true;
      goto cleanup;
    }
    answer = predicate_query_implication_branch(
        bounds, antecedent, predicate->u.assoc.args[consequent_index],
        &branch_limited);
    if (branch_limited) {
      if (limited)
        *limited = true;
      answer = IXS_CHECK_UNKNOWN;
      goto cleanup;
    }
    if (answer == IXS_CHECK_TRUE || bounds->oom)
      goto cleanup;
  }
...
static bool predicate_finite_collect_symbols(ixs_bounds *bounds,
                                             ...
                                             size_t *symbol_count) {
  ixs_arena *arena = &bounds->work_arena;
...
static bool equivalence_memo_grow(equivalence_state *state) {
  ...
  if (next_capacity <= state->memo_capacity ||
      next_capacity > SIZE_MAX / sizeof(*grown))
    return false;
  grown = ixs_arena_alloc(&state->bounds->work_arena,
                          next_capacity * sizeof(*grown), sizeof(void *));
  if (!grown)
    return false;
  memset(grown, 0, next_capacity * sizeof(*grown));
...
static void equivalence_state_init(equivalence_state *state, ixs_ctx *ctx,
  ...
  state->ctx = ctx;
  state->bounds = bounds;
  state->memo_mark = ixs_arena_save(&bounds->work_arena);
}

static void equivalence_state_destroy(equivalence_state *state) {
  ixs_arena_restore(&state->bounds->work_arena, state->memo_mark);
}
...
static equivalence_query_status equivalence_query_bounds_detail_at_depth(
    ...
    unsigned bounded_subproof_depth, ixs_check_result *result);
 ...
 * the parent's bounds, while each branch has stronger bounds.  For C cases
 * this performs O(C) forks and C bounded equivalence queries; it never scans
 * unrelated context state. */
static ixs_check_result
equivalence_piecewise_partitions(equivalence_state *state, ixs_node *piecewise,
                                 ixs_node *scalar) {
  ixs_arena_mark outer_mark;
  ixs_bounds remaining;
  bool remaining_ready = false;
  ...
  if (!piecewise || piecewise->tag != IXS_PIECEWISE || !scalar ||
      piecewise->u.pw.ncases == 0u || !piecewise->u.pw.cases ||
      state->bounded_subproof_depth >= EQUIVALENCE_BOUNDED_SUBPROOF_DEPTH)
    return IXS_CHECK_UNKNOWN;

  outer_mark = ixs_arena_save(state->bounds->scratch);
  if (!ixs_bounds_fork(&remaining, state->bounds)) {
    state->oom = true;
    goto cleanup;
  }
  remaining_ready = true;
  ...
  for (i = 0; i < piecewise->u.pw.ncases; i++) {
    ...
    if (ixs_bounds_check_defined(&remaining, condition) != IXS_CHECK_TRUE)
      goto cleanup;
  ...
  if (!covered && ixs_bounds_has_empty(&remaining))
    covered = true;
  result = covered ? IXS_CHECK_TRUE : IXS_CHECK_UNKNOWN;

cleanup:
  bounds_destroy_if_initialized(&remaining, remaining_ready);
  ixs_arena_restore(state->bounds->scratch, outer_mark);
  return state->limited || state->invalid || state->oom ? IXS_CHECK_UNKNOWN
                                                        : result;
}

static bool equivalence_same_context_shape(ixs_node *lhs, ixs_node *rhs,
                                           uint32_t *child_count) {
...
equivalence_context_seen_grow(equivalence_state *state,
  ...
  if (capacity <= worklist->seen_capacity ||
      capacity > SIZE_MAX / sizeof(*grown))
    goto oom;
  grown = ixs_arena_alloc(&state->bounds->work_arena, capacity * sizeof(*grown),
                          sizeof(void *));
  if (!grown)
    goto oom;
  memset(grown, 0, capacity * sizeof(*grown));
...
static bool equivalence_context_enqueue(equivalence_state *state,
  ...
  if (worklist->count == worklist->capacity) {
    ...
    grown = ixs_arena_grow(&state->bounds->work_arena, worklist->pairs,
...
static bool equivalence_cancel_common_add(equivalence_state *state,
  ...
  original_size = lhs->u.add.nterms + rhs->u.add.nterms;
  residual_size = equivalence_add_relation_size(relation_lhs) +
                  equivalence_add_relation_size(relation_rhs);
...
/* Iterative congruence is O(N) in the paired DAG. Associative matching may
 * start O(A^2) bounded subproofs after exact argument matches. */
static ixs_check_result equivalence_same_context(equivalence_state *state,
  ...
  if (!equivalence_same_context_shape(lhs, rhs, &child_count) ||
  ...
  while (worklist.count > 0u) {
    ...
    if (simplified_lhs->tag == simplified_rhs->tag &&
      ...
      if (equivalence_match_associative_context(
    ...
    }
    {
      ...
      if (equivalence_cancel_common_add(state, simplified_lhs, simplified_rhs,
        ...
        if (!equivalence_context_enqueue(state, &worklist, lhs_residual,
                                         rhs_residual))
          return IXS_CHECK_UNKNOWN;
        continue;
      }
    ...
    if (!equivalence_same_context_shape(simplified_lhs, simplified_rhs,
                                        &child_count))
      return IXS_CHECK_UNKNOWN;
    for (i = 0; i < child_count; i++)
      if (!equivalence_context_enqueue(state, &worklist,
                                       defined_child_at(simplified_lhs, i),
                                       defined_child_at(simplified_rhs, i)))
        return IXS_CHECK_UNKNOWN;
  }
  return IXS_CHECK_TRUE;
}
...
static bool equivalence_integer_content(ixs_node *expr, uint64_t *content) {
  uint64_t result = 0;
  uint32_t i;
  ...
  if (expr->tag == IXS_ADD) {
    ...
    for (i = 0; i < expr->u.add.nterms; i++) {
      ...
      magnitude = bounds_int64_magnitude(p);
      if (magnitude != 0)
        result = result == 0 ? magnitude : bounds_u64_gcd(result, magnitude);
    }
    *content = result;
    return true;
  }
  if (expr->tag == IXS_MUL) {
    ...
    if (q != 1)
      return false;
    *content = bounds_int64_magnitude(p);
    return true;
  }
  if (expr->tag == IXS_INT) {
    *content = bounds_int64_magnitude(expr->u.ival);
    return true;
  }
  *content = 1;
  return true;
...
/* Equality is invariant under cancellation of a shared nonzero integer
 * content.  Extract content only from canonical top-level coefficients, then
...
equivalence_cancel_integer_content(equivalence_state *state, ixs_node *lhs,
  ...
  ixs_node *expanded_rhs;
  uint64_t lhs_content;
  uint64_t rhs_content;
  uint64_t content;
  equivalence_build_status lhs_status;
  equivalence_build_status rhs_status;
  bool lhs_limited = false;
  bool rhs_limited = false;

  if (!equivalence_integer_content(lhs, &lhs_content) ||
      !equivalence_integer_content(rhs, &rhs_content))
    return IXS_CHECK_UNKNOWN;
  ...
  if (ixs_node_is_sentinel(scaled_lhs) || ixs_node_is_sentinel(scaled_rhs) ||
      (scaled_lhs == lhs && scaled_rhs == rhs))
    return IXS_CHECK_UNKNOWN;
...
/* Equal positive literal Mod contexts need only congruent integer dividends;
 * their dividends need not be equal. */
...
static bool bounds_get_integer_range(ixs_bounds *bounds, ixs_node *expr,
                                     ixs_integer_range_result *out) {
  ixs_interval interval;
...
static bool bounds_integer_enclosure(ixs_bounds *bounds, ixs_node *expr,
  ...
  if (!bounds_get_integer_range(bounds, expr, &range) || !range.has_lower ||
      !range.has_upper)
    return false;
  *lower = range.lower;
  *upper = range.upper;
...
IXS_STATIC bool ixs_bounds_mod_shift_stays_in_residue(ixs_bounds *bounds,
                                                      ...
                                                      ixs_node *delta) {
  ixs_integer_range_result range;
  uint64_t delta_stride;
  uint64_t increment;
  uint64_t modulus;
  uint64_t residue;
  if (!bounds_get_integer_range(bounds, delta, &range) || !range.has_lower ||
      !range.has_upper)
    return false;
  ...
  /* The smallest positive stride multiple above delta's finite range is
   * merely an expression-derived candidate bucket.
   * Structural stride analysis can conservatively lose fact-backed alignment
   * in products, so the full fact domain must still prove that both the base
   * and denominator are divisible by that candidate. */
  if (!bounds_known_stride(bounds, delta, &delta_stride)) {
    if (bounds->oom)
      return false;
    delta_stride = 1u;
  }
  if (delta_stride == 0)
    delta_stride = 1u;
  if (delta_stride > (uint64_t)INT64_MAX ||
      (uint64_t)range.upper == (uint64_t)INT64_MAX)
    return false;
  modulus = (uint64_t)range.upper + 1u;
  increment = modulus % delta_stride;
  if (increment != 0) {
    increment = delta_stride - increment;
    if (modulus > (uint64_t)INT64_MAX - increment)
      return false;
    modulus += increment;
  }
...
/* For an integer comparison, threshold is the inclusive edge of its true
 * half-line. If anchor - moving is in [delta_lower, delta_upper], the moving
 * edge in anchor coordinates is in
 * [moving_threshold + delta_lower, moving_threshold + delta_upper]. The
 * interval below is the exact union of the disagreement cuts admitted by
 * that enclosure. */
static bool equivalence_ordered_bounded_delta_direction(
    equivalence_state *state, ixs_node *anchor, bool lower,
    int64_t anchor_threshold, int64_t moving_threshold, int64_t delta_lower,
    int64_t delta_upper) {
  int64_t mapped_lower;
  int64_t mapped_upper;
  int64_t lo;
  int64_t hi;

  if (!ixs_safe_add(moving_threshold, delta_lower, &mapped_lower) ||
      !ixs_safe_add(moving_threshold, delta_upper, &mapped_upper))
    return false;
  lo = anchor_threshold < mapped_lower ? anchor_threshold : mapped_lower;
  hi = anchor_threshold < mapped_upper ? mapped_upper : anchor_threshold;
  if (lower) {
    if (!ixs_safe_sub(hi, 1, &hi))
      return false;
  } else if (!ixs_safe_add(lo, 1, &lo)) {
    return false;
  }
  return equivalence_no_reachable_integer(state, anchor, lo, hi);
}

static ixs_check_result equivalence_ordered_bounded_delta(
    equivalence_state *state, ixs_node *lhs, ixs_node *rhs, bool lower,
    int64_t left_threshold, int64_t right_threshold) {
  ixs_integer_range_result range;
  ixs_node *difference;
  int64_t reverse_lower;
  int64_t reverse_upper;
  ...
  if (state->oom ||
      !bounds_get_integer_range(state->bounds, difference, &range) ||
      !range.has_lower || !range.has_upper) {
    if (state->bounds->oom)
      state->oom = true;
    return IXS_CHECK_UNKNOWN;
  }
  if (equivalence_ordered_bounded_delta_direction(
          state, lhs, lower, left_threshold, right_threshold, range.lower,
          range.upper))
    return IXS_CHECK_TRUE;

  /* Equivalence queries canonicalize operand order. Try the opposite anchor
   * as well: one side may carry the useful congruence while the other side's
   * bounded displacement deliberately does not. */
  if (ixs_safe_neg(range.upper, &reverse_lower) &&
      ixs_safe_neg(range.lower, &reverse_upper) &&
      equivalence_ordered_bounded_delta_direction(
          state, rhs, lower, right_threshold, left_threshold, reverse_lower,
          reverse_upper))
    return IXS_CHECK_TRUE;
  return IXS_CHECK_UNKNOWN;
}
...
equivalence_ordered_comparisons(equivalence_state *state, ixs_node *lhs,
  ...
  if (equivalence_ordered_bounded_delta(
          state, lhs->u.binary.lhs, rhs->u.binary.lhs, left_lower,
          left_threshold, right_threshold) == IXS_CHECK_TRUE)
    return IXS_CHECK_TRUE;
 ...
 * canonical range.  The bounded child proof composes algebraic identities;
 * it does not inspect values from a finite domain. */
  ...
  *mod = NULL;
  for (i = 0; i < expr->u.add.nterms; i++) {
    if (expr->u.add.terms[i].term->tag != IXS_MOD)
      continue;
    ixs_node_get_rat(expr->u.add.terms[i].coeff, &p, &q);
    /* Other Mod terms are part of the arbitrary residual.  Only a positive
     * integer coefficient greater than one identifies the scaled term. */
    if (q != 1 || p <= 1)
      continue;
    if (*mod)
      return false;
...
static ixs_check_result equivalence_isolate_linear_mod(equivalence_state *state,
  ...
  /* Every Mod term gives an exact rearrangement of difference == 0.  Try at
   * most one bounded subproof per term instead of rejecting a sum merely
   * because its other side also contains a Mod. */
  for (i = 0; i < difference->u.add.nterms; i++) {
...
equivalence_mod_quotient_identity(equivalence_state *state, ixs_node *lhs,
  ...
  if (!difference)
    return IXS_CHECK_UNKNOWN;
  if (equivalence_isolate_linear_mod(state, difference, depth) ==
      IXS_CHECK_TRUE)
    return IXS_CHECK_TRUE;
  ...
  for (i = 0; i < sizeof(tags) / sizeof(tags[0]); i++) {
    ...
    if (!equivalence_split_affine_round(state, lhs, tags[i], &lhs_argument,
        ...
        !equivalence_split_affine_round(state, rhs, tags[i], &rhs_argument,
                                        &rhs_residual) ||
        ixs_bounds_check_integer_valued(state->bounds, lhs_residual) !=
            IXS_CHECK_TRUE ||
        ixs_bounds_check_integer_valued(state->bounds, rhs_residual) !=
            IXS_CHECK_TRUE ||
...
/* Extracting a radix digit can be expressed either before or after taking a
 * Euclidean quotient:
 *
 *   floor(Mod(A, M) / D) = Mod(floor(A / D), M / D)
 *
 * for integer A and positive integer D dividing M.  Keep this identity in the
 * equivalence prover rather than changing the global simplification normal
 * form: it is a proof relation between two equally useful representations.
 */
static ixs_check_result equivalence_floor_mod_quotient_digit_direction(
    equivalence_state *state, ixs_node *digit, ixs_node *quotient_digit,
    unsigned depth) {
  ixs_node *digit_numerator;
  ixs_node *digit_denominator;
  ixs_node *base;
  ixs_node *quotient_arg;
  ixs_node *quotient;
  ixs_node *quotient_modulus_node;
  ixs_node *projected;
  ixs_quotient_parts_status digit_status;
  int64_t divisor;
  int64_t modulus;
  int64_t quotient_modulus;

  if (!digit || !quotient_digit || digit->tag != IXS_FLOOR)
    return IXS_CHECK_UNKNOWN;

  digit_status = simp_decompose_exact_quotient(
      state->ctx, digit->u.unary.arg, &digit_numerator, &digit_denominator);
  if (digit_status == IXS_QUOTIENT_PARTS_OOM) {
    state->oom = true;
    return IXS_CHECK_UNKNOWN;
  }
  if (digit_status != IXS_QUOTIENT_PARTS_MATCH ||
      digit_numerator->tag != IXS_MOD ||
      digit_numerator->u.binary.rhs->tag != IXS_INT ||
      digit_denominator->tag != IXS_INT)
    return IXS_CHECK_UNKNOWN;

  base = digit_numerator->u.binary.lhs;
  divisor = digit_denominator->u.ival;
  modulus = digit_numerator->u.binary.rhs->u.ival;
  if (divisor <= 0 || modulus <= 0 || modulus % divisor != 0 ||
      ixs_bounds_check_integer_valued(state->bounds, base) != IXS_CHECK_TRUE)
    return IXS_CHECK_UNKNOWN;
  quotient_modulus = modulus / divisor;
  quotient_modulus_node = ixs_node_int(state->ctx, quotient_modulus);
  if (!quotient_modulus_node ||
      equivalence_build_div_const(state, base, digit_denominator,
                                  &quotient_arg) != EQUIVALENCE_BUILD_OK) {
    if (!quotient_modulus_node)
      state->oom = true;
    return IXS_CHECK_UNKNOWN;
  ...
  if (ixs_node_is_sentinel(projected))
    return IXS_CHECK_UNKNOWN;
  return projected == quotient_digit
             ? IXS_CHECK_TRUE
             : equivalence_bounded_core(state, projected, quotient_digit,
                                        depth + 1u);
}
...
static ixs_check_result
equivalence_floor_quotient_direction(equivalence_state *state,
                                     ixs_node *rounded, ixs_node *quotient) {
  ixs_node *numerator;
  ixs_node *denominator;
  ...
  if (status != IXS_QUOTIENT_PARTS_MATCH ||
      ixs_bounds_check_integer_valued(state->bounds, numerator) !=
          IXS_CHECK_TRUE ||
      ixs_bounds_check_integer_valued(state->bounds, denominator) !=
          IXS_CHECK_TRUE ||
      ixs_bounds_check_integer_valued(state->bounds, quotient) !=
          IXS_CHECK_TRUE ||
      equivalence_build_scale(state, denominator, quotient, &scaled_quotient) !=
          EQUIVALENCE_BUILD_OK ||
      equivalence_build_sub(state, numerator, scaled_quotient, &remainder) !=
          EQUIVALENCE_BUILD_OK ||
      ixs_node_is_sentinel(remainder) ||
...
static ixs_check_result equivalence_floor_quotient(equivalence_state *state,
                                                   ...
                                                   ixs_node *rhs) {
  ixs_check_result result =
      equivalence_floor_quotient_direction(state, lhs, rhs);
  if (result != IXS_CHECK_UNKNOWN)
    return result;
  return equivalence_floor_quotient_direction(state, rhs, lhs);
}
...
static ixs_check_result equivalence_core_impl(equivalence_state *state,
  ...
  /* Preserve both original comparison shapes long enough for their algebraic
   * relation to be proved.  Fact-conditioned simplification may otherwise
...
equivalence_query_bounds_detail(ixs_bounds *bounds, ixs_ctx *ctx, ixs_node *lhs,
  ...
  return equivalence_query_bounds_detail_at_depth(bounds, ctx, lhs, rhs, 0u,
                                                  result);
}
...
facts_query_predicate_shape(ixs_bounds *bounds, ixs_node *predicate,
                            bool try_finite_domain) {
  ixs_fact_check_result result = {IXS_FACT_QUERY_COMPLETE, IXS_CHECK_UNKNOWN};
  bool limited = false;

  result.check = predicate_query_eval_detail(bounds, predicate, &limited);
  if (!limited && result.check == IXS_CHECK_UNKNOWN)
    result.check = predicate_query_complement_subsumption(bounds, predicate);
  if (!limited && result.check == IXS_CHECK_UNKNOWN)
    result.check = predicate_query_implication(bounds, predicate, &limited);
  if (limited) {
    result.status = IXS_FACT_QUERY_LIMITED;
    result.check = IXS_CHECK_UNKNOWN;
  } else if (try_finite_domain && result.check == IXS_CHECK_UNKNOWN) {
    result.check = predicate_query_finite_domain(bounds, predicate, NULL);
  }
  return result;
}
...
static ixs_fact_check_result facts_query_check_predicate(ixs_facts *facts,
  ...
  scratch_saved = true;
  result =
      facts_query_normalized_predicate(facts, ctx, predicate, &source_fallback);
  if (result.status != IXS_FACT_QUERY_COMPLETE || !source_fallback)
    goto cleanup;

  /* Some structural predicate proofs intentionally depend on the source
...
static ixs_check_result bounds_check_with_equivalence(ixs_bounds *bounds,
                                                      ixs_node *expr) {
  ixs_check_result result = ixs_bounds_check(bounds, expr);
  if (result == IXS_CHECK_UNKNOWN && expr->tag == IXS_CMP &&
      (expr->u.binary.cmp_op == IXS_CMP_EQ ||
       expr->u.binary.cmp_op == IXS_CMP_NE))
    result = bounds_check_equivalence_atom(bounds, expr);
  return result;
}
...
static ixs_fact_check_result facts_query_check(ixs_facts *facts,
  ...
  if (!ixs_node_is_known_true(expr) && !ixs_node_is_known_false(expr) &&
      expr->tag != IXS_CMP) {
    ixs_ctx_push_error(ctx, "check: expression must be a comparison");
    goto cleanup;
  }
  if (ixs_bounds_has_empty(&facts->bounds)) {
    result.status = IXS_FACT_QUERY_COMPLETE;
  ...
  /* Exact expression facts belong to the source shape. Simplifying first can
   * replace that key with an equal range-normalized expression which has no
   * direct override. The source check is cheap and the normalized proof below
   * remains the fallback for derived relations. */
  result.check = bounds_check_with_equivalence(&facts->bounds, expr);
  ...
  result.check = bounds_check_with_equivalence(&facts->bounds, normalized);
  result.status = IXS_FACT_QUERY_COMPLETE;
...
static ixs_range_query_result facts_query_range(ixs_facts *facts,
  ...
  if (integer_valued == IXS_CHECK_TRUE) {
    if (!bounds_get_integer_range(&facts->bounds, expr, &integer_range))
      goto cleanup;
