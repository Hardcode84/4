...
If the checked integer/fractional split is not representable, the rule leaves
the rounding node unchanged.  Its recursive tail is entered only after an
integer constant or term was actually removed from the remainder.
...
- **Public integrality queries**: `ixs_node_is_integer_valued` is a
  ...
  value. `Mod(A, D)` recursively uses the same fact-backed proof for both
