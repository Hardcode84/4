...
test_ixs_check_predicate_facts(ixs_facts *facts, const ixs_node *predicate) {
  return test_ixs_fact_check_value(ixs_check_predicate_facts(facts, predicate));
}

static inline ixs_check_result
test_ixs_check_consistent_facts(ixs_facts *facts) {
...
static inline ixs_check_result
test_ixs_equivalent_modulo_pow2_facts(ixs_facts *facts, const ixs_node *lhs,
                                      const ixs_node *rhs, unsigned bits) {
...
test_ixs_check_integer_valued_facts(ixs_facts *facts, const ixs_node *expr) {
  return test_ixs_fact_check_value(ixs_check_integer_valued_facts(facts, expr));
}
...
static inline bool
test_ixs_decompose_exact_quotient_facts(ixs_facts *facts, const ixs_node *expr,
  ...
  return numerator && denominator && result.status == IXS_FACT_QUERY_COMPLETE &&
         result.available;
}
...
static inline ixs_check_result test_ixs_check_invariant_under_step_facts(
    ixs_facts *facts, const ixs_node *expr, const ixs_node *symbol,
    const ixs_node *step) {
  return test_ixs_fact_check_value(
      ixs_check_invariant_under_step_facts(facts, expr, symbol, step));
}

static inline bool
test_ixs_decompose_cyclic_facts(ixs_facts *facts, const ixs_node *expr,
                                const ixs_node *symbol,
