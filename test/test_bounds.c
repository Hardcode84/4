...
static void test_bounds_divisibility_through_mod(void) {
  ixs_ctx *ctx = ixs_ctx_create();
  ixs_bounds b;
  ixs_node *x = ixs_sym(ctx, "x");
  ixs_node *integer_x = ixs_floor(ctx, x);
  ixs_node *aligned =
      ixs_mod(ctx, ixs_mul(ctx, ixs_int(ctx, 16), integer_x), ixs_int(ctx, 32));
  ixs_node *mod =
      ixs_mod(ctx, ixs_div(ctx, x, ixs_int(ctx, 2)), ixs_int(ctx, 32));
  ixs_node *nondividing =
      ixs_mod(ctx, ixs_mul(ctx, ixs_int(ctx, 16), integer_x), ixs_int(ctx, 24));

  CHECK(ixs_node_tag(aligned) == IXS_MOD);
  CHECK(ixs_node_tag(mod) == IXS_MOD);
  CHECK(ixs_bounds_init(&b, ixs_test_scratch(ctx)));
  CHECK(ixs_bounds_is_known_divisible(&b, aligned, 16));
  CHECK(!ixs_bounds_is_known_divisible(&b, mod, 16));
  CHECK(!ixs_bounds_is_known_divisible(&b, nondividing, 16));
...
static void test_public_range_grouped_mod_congruence(void) {
  ...
  CHECK(ctx && domain && separate_domain && aligned_domain && positive &&
        negative && aligned && nested && separate && noninteger && nontotal &&
        facts && separate_facts && aligned_facts);
  ...
  /* Equal-shaped Mod terms with distinct representatives remain independent. */
...
static void test_public_euclidean_division_identities(void) {
  ixs_ctx *ctx = ixs_ctx_create();
  ixs_node *x = ixs_sym(ctx, "euclidean_x");
  ixs_node *q = ixs_sym(ctx, "euclidean_q");
  ixs_node *zero = ixs_int(ctx, 0);
  ixs_node *four = ixs_int(ctx, 4);
  ixs_node *eight = ixs_int(ctx, 8);
  ixs_node *remainder = ixs_sub(ctx, x, ixs_mul(ctx, four, q));
  ixs_node *quotient = ixs_floor(ctx, ixs_div(ctx, x, four));
  ixs_node *residue = ixs_mod(ctx, x, four);
  ixs_node *partitioned =
      ixs_floor(ctx, ixs_div(ctx, ixs_sub(ctx, x, residue), eight));
  ixs_node *direct = ixs_floor(ctx, ixs_div(ctx, x, eight));
  ixs_facts *facts = ixs_facts_create(ctx);
  ixs_facts *integer_only = ixs_facts_create(ctx);

  CHECK(ctx && x && q && zero && four && eight && remainder && quotient &&
        residue && partitioned && direct && facts && integer_only);
  CHECK(ixs_facts_assume_pred(facts,
                              ixs_cmp(ctx, x, IXS_CMP_EQ, ixs_floor(ctx, x))));
  CHECK(ixs_facts_assume_pred(facts,
                              ixs_cmp(ctx, q, IXS_CMP_EQ, ixs_floor(ctx, q))));
  CHECK(
      ixs_facts_assume_pred(facts, ixs_cmp(ctx, remainder, IXS_CMP_GE, zero)));
  CHECK(
      ixs_facts_assume_pred(facts, ixs_cmp(ctx, remainder, IXS_CMP_LT, four)));
  CHECK(ixs_facts_assume_pred(facts, ixs_cmp(ctx, x, IXS_CMP_GE, zero)));

  CHECK(test_ixs_equivalent_facts(facts, quotient, q) == IXS_CHECK_TRUE);
  CHECK(test_ixs_equivalent_facts(facts, direct, partitioned) ==
        IXS_CHECK_TRUE);
  CHECK(test_ixs_check_facts(
            facts, ixs_cmp(ctx, ixs_sub(ctx, x, residue), IXS_CMP_GE, zero)) ==
        IXS_CHECK_TRUE);
  CHECK(test_ixs_check_facts(
            facts, ixs_cmp(ctx, ixs_sub(ctx, residue, x), IXS_CMP_LE, zero)) ==
        IXS_CHECK_TRUE);

  CHECK(ixs_facts_assume_pred(integer_only,
                              ixs_cmp(ctx, x, IXS_CMP_EQ, ixs_floor(ctx, x))));
  CHECK(ixs_facts_assume_pred(integer_only,
                              ixs_cmp(ctx, q, IXS_CMP_EQ, ixs_floor(ctx, q))));
  CHECK(test_ixs_equivalent_facts(integer_only, quotient, q) ==
        IXS_CHECK_UNKNOWN);
...
static void test_public_range_associative_many(void) {
  ...
  args[0] = ixs_int(ctx, 1);
  args[1] = x;
  args[2] = y;
  CHECK(ixs_range(ctx, ixs_max_many(ctx, 3, args), NULL, 0, &r));
  CHECK(r.has_lower && r.lower_p == 1 && r.lower_q == 1);
  CHECK(!r.has_upper);
  args[0] = ixs_int(ctx, -1);
  CHECK(ixs_range(ctx, ixs_min_many(ctx, 3, args), NULL, 0, &r));
  CHECK(!r.has_lower);
  CHECK(r.has_upper && r.upper_p == -1 && r.upper_q == 1);
  ...
  sum = ixs_add(ctx, ixs_xor(ctx, overlap[0], overlap[1]), overlap[2]);
  CHECK(ixs_simplify(ctx, ixs_xor_many(ctx, 3, overlap), bounds, 6) == sum);
...
static void
test_public_fact_integrality_rejects_modular_noninteger_factor(void) {
  ixs_ctx *ctx = ixs_ctx_create();
  ixs_node *x = ixs_sym(ctx, "modular_noninteger_x");
  ixs_node *y = ixs_sym(ctx, "modular_noninteger_y");
  ixs_node *zero = ixs_int(ctx, 0);
  ixs_node *two = ixs_int(ctx, 2);
  ixs_node *maximum = ixs_max(ctx, ixs_rat(ctx, 3, 2), x);
  ixs_node *values[2] = {ixs_mul(ctx, two, maximum), y};
  ixs_node *conditions[2] = {ixs_cmp(ctx, x, IXS_CMP_GE, y), ixs_true(ctx)};
  ixs_node *piecewise = ixs_pw(ctx, 2, values, conditions);
  ixs_node *scaled = ixs_div(ctx, piecewise, two);
  ixs_node *floored = ixs_floor(ctx, scaled);
  ixs_node *even = ixs_cmp(ctx, ixs_mod(ctx, y, two), IXS_CMP_EQ, zero);
  ixs_facts *facts = ixs_facts_create(ctx);
  ixs_node *simplified;

  CHECK(ctx && x && y && zero && two && maximum && values[0] && piecewise &&
        scaled && floored && even && facts);
  CHECK(ixs_facts_assume_pred(facts, even));
  CHECK(test_ixs_check_divisible_facts(facts, piecewise, 2) ==
        IXS_CHECK_UNKNOWN);
  CHECK(test_ixs_check_integer_valued_facts(facts, scaled) ==
        IXS_CHECK_UNKNOWN);
  simplified = test_ixs_simplify_facts(facts, floored);
  CHECK(simplified && ixs_node_tag(simplified) == IXS_FLOOR);
...
static void test_public_fact_integrality_nested_mod_cancellation(void) {
  static const char exact_text[] = "1/4*(8 + Mod(x, 8) - Mod(Mod(x, 8), 4))";
  static const char base_text[] = "8 + Mod(x, 8) - Mod(Mod(x, 8), 4)";
  static const char noncancelling_text[] =
      "1/4*(8 + Mod(x, 8) - Mod(Mod(x, 8), 3))";
  static const char negative_modulus_text[] =
      "1/4*(8 + Mod(x, 8) - Mod(Mod(x, 8), -4))";
  static const char noninteger_dividend_text[] =
      "1/4*(8 + Mod(x/2, 8) - Mod(Mod(x/2, 8), 4))";
  ixs_ctx *ctx = ixs_ctx_create();
  ...
  ixs_node *domain =
      ixs_parse_pred(ctx, "x == floor(x)", strlen("x == floor(x)"));
  ixs_facts *facts = ixs_facts_create(ctx);
...
static void test_public_fact_integrality_scaled_xor(void) {
  static const char expression_text[] = "1/2*xor(2*Mod(x, 4), 6*Mod(y, 4))";
  ixs_ctx *ctx = ixs_ctx_create();
  ixs_node *expression =
...
static void test_generic_piecewise_scalar_equivalence_by_partitions(void) {
  ixs_ctx *ctx = ixs_ctx_create();
  ixs_node *x = ixs_sym(ctx, "x");
  ixs_node *zero = ixs_int(ctx, 0);
  ixs_node *condition = ixs_cmp(ctx, x, IXS_CMP_EQ, zero);
  ixs_node *values[2] = {x, zero};
  ixs_node *conditions[2] = {condition, ixs_true(ctx)};
  ixs_node *expression = ixs_pw(ctx, 2, values, conditions);
  ixs_facts *facts = ixs_facts_create(ctx);

  CHECK(ctx && x && zero && condition && expression && facts);
  CHECK(ixs_node_tag(expression) == IXS_PIECEWISE);
  CHECK(test_ixs_equivalent_facts(facts, expression, zero) == IXS_CHECK_TRUE);
  CHECK(test_ixs_check_predicate_facts(
            facts, ixs_cmp(ctx, expression, IXS_CMP_EQ, zero)) ==
        IXS_CHECK_TRUE);
...
static void
test_generic_piecewise_scalar_equivalence_rejects_incomplete_proof(void) {
  static const char domain_text[] = "x >= -1 & x <= 1";
  ixs_ctx *ctx = ixs_ctx_create();
  ixs_node *x = ixs_sym(ctx, "x");
  ixs_node *zero = ixs_int(ctx, 0);
  ixs_node *one = ixs_int(ctx, 1);
  ixs_node *condition = ixs_cmp(ctx, x, IXS_CMP_LT, zero);
  ixs_node *domain = ixs_parse_pred(ctx, domain_text, strlen(domain_text));
  ixs_node *unequal_values[2] = {one, zero};
  ixs_node *unequal_conditions[2] = {condition, ixs_true(ctx)};
  ixs_node *unequal = ixs_pw(ctx, 2, unequal_values, unequal_conditions);
  ixs_pwcase uncovered_case = {zero, condition};
  ixs_node *uncovered = ixs_node_pw(ctx, 1, &uncovered_case);
  ixs_facts *facts = ixs_facts_create(ctx);

  CHECK(ctx && x && zero && one && condition && domain && unequal &&
        uncovered && facts);
  CHECK(ixs_facts_assume_pred(facts, domain));
  CHECK(test_ixs_equivalent_facts(facts, unequal, zero) == IXS_CHECK_UNKNOWN);
  CHECK(test_ixs_equivalent_facts(facts, uncovered, zero) == IXS_CHECK_UNKNOWN);
...
static void test_public_predicate_tree_query(void) {
  ...
  ixs_node *z = ixs_sym(ctx, "predicate_z");
  ...
  ixs_node *activity_values[2] = {x_nonnegative, y_small};
  ixs_node *activity_conditions[2] = {
      ixs_cmp(ctx, z, IXS_CMP_GE, ixs_int(ctx, 0)), ixs_true(ctx)};
  ixs_node *activity = ixs_pw(ctx, 2u, activity_values, activity_conditions);
  ixs_node *partial_activity_conditions[2] = {partial_pred, ixs_true(ctx)};
  ixs_node *partial_activity =
      ixs_pw(ctx, 2u, activity_values, partial_activity_conditions);
  ixs_node *guarded_relation =
      ixs_and(ctx, y_small, ixs_or(ctx, x_nonnegative, ixs_not(ctx, activity)));
  ixs_node *guarded_implication = ixs_or(
      ctx, x_nonnegative,
      ixs_or(ctx, ixs_not(ctx, activity), ixs_not(ctx, guarded_relation)));
  ixs_node *partial_guarded_relation = ixs_and(
      ctx, y_small, ixs_or(ctx, x_nonnegative, ixs_not(ctx, partial_activity)));
  ixs_node *partial_guarded_implication =
      ixs_or(ctx, x_nonnegative,
             ixs_or(ctx, ixs_not(ctx, partial_activity),
                    ixs_not(ctx, partial_guarded_relation)));
  ixs_node *non_implication =
      ixs_or(ctx, y_small,
             ixs_or(ctx, ixs_not(ctx, activity),
                    ixs_not(ctx, ixs_and(ctx, x_nonnegative,
                                         ixs_or(ctx, x_nonnegative,
                                                ixs_not(ctx, activity))))));
  ixs_node *subsumed_or =
      ixs_or(ctx, x_nonnegative,
             ixs_not(ctx, ixs_and(ctx, x_nonnegative, partial_pred)));
  ixs_node *subsumed_and =
      ixs_and(ctx, x_nonnegative,
              ixs_not(ctx, ixs_or(ctx, x_nonnegative, partial_pred)));
  ...
  CHECK(test_ixs_check_predicate_facts(no_domain, partial_implication) ==
        IXS_CHECK_UNKNOWN);
  CHECK(ixs_node_tag(activity) == IXS_PIECEWISE);
  CHECK(test_ixs_check_predicate_facts(no_domain, guarded_implication) ==
        IXS_CHECK_TRUE);
  CHECK(test_ixs_check_predicate_facts(
            no_domain, partial_guarded_implication) == IXS_CHECK_UNKNOWN);
  CHECK(test_ixs_check_predicate_facts(no_domain, non_implication) ==
        IXS_CHECK_UNKNOWN);
  CHECK(test_ixs_check_predicate_facts(no_domain, subsumed_or) ==
        IXS_CHECK_UNKNOWN);
  CHECK(test_ixs_check_predicate_facts(no_domain, subsumed_and) ==
        IXS_CHECK_UNKNOWN);
  ...
  CHECK(test_ixs_check_predicate_facts(defined, guarded_true) ==
        IXS_CHECK_TRUE);
  CHECK(test_ixs_check_predicate_facts(defined, subsumed_or) == IXS_CHECK_TRUE);
  CHECK(test_ixs_check_predicate_facts(defined, subsumed_and) ==
        IXS_CHECK_FALSE);
  CHECK(test_ixs_check_predicate_facts(defined, partial_guarded_implication) ==
        IXS_CHECK_TRUE);
...
static void test_public_equivalence_ordered_bounded_delta(void) {
  ixs_ctx *ctx = ixs_ctx_create();
  ixs_node *base = ixs_sym(ctx, "ordered_delta_base");
  ixs_node *shift = ixs_sym(ctx, "ordered_delta_shift");
  ixs_node *large = ixs_sym(ctx, "ordered_delta_large");
  ixs_node *small = ixs_sym(ctx, "ordered_delta_small");
  ixs_node *zero = ixs_int(ctx, 0);
  ixs_node *seven = ixs_int(ctx, 7);
  ixs_node *eight = ixs_int(ctx, 8);
  ixs_node *shifted = ixs_add(ctx, base, shift);
  ixs_node *large_shifted = ixs_add(ctx, base, large);
  ixs_node *small_shifted = ixs_add(ctx, base, small);
  ixs_node *base_grid =
      ixs_cmp(ctx, ixs_mod(ctx, base, eight), IXS_CMP_EQ, zero);
  ixs_node *shift_lower = ixs_cmp(ctx, shift, IXS_CMP_GE, zero);
  ixs_node *shift_upper = ixs_cmp(ctx, shift, IXS_CMP_LE, seven);
  ixs_facts *grid = ixs_facts_create(ctx);
  ixs_facts *crossing = ixs_facts_create(ctx);
  ixs_facts *ungridded = ixs_facts_create(ctx);
  ixs_facts *lower_only = ixs_facts_create(ctx);
  ixs_facts *upper_only = ixs_facts_create(ctx);
  ixs_facts *large_facts = ixs_facts_create(ctx);
  ixs_facts *small_facts = ixs_facts_create(ctx);
  size_t errors;

  CHECK(ctx && base && shift && large && small && zero && seven && eight &&
        shifted && large_shifted && small_shifted && base_grid && shift_lower &&
        shift_upper && grid && crossing && ungridded && lower_only &&
        upper_only && large_facts && small_facts);

  CHECK(ixs_facts_assume_pred(grid, base_grid));
  CHECK(ixs_facts_assume_pred(grid, shift_lower));
  CHECK(ixs_facts_assume_pred(grid, shift_upper));
  CHECK(test_ixs_equivalent_facts(grid, ixs_cmp(ctx, shifted, IXS_CMP_LT, zero),
                                  ixs_cmp(ctx, base, IXS_CMP_LT, zero)) ==
        IXS_CHECK_TRUE);
  CHECK(test_ixs_equivalent_facts(grid, ixs_cmp(ctx, base, IXS_CMP_LT, zero),
                                  ixs_cmp(ctx, shifted, IXS_CMP_LT, zero)) ==
        IXS_CHECK_TRUE);
  CHECK(test_ixs_equivalent_facts(grid, ixs_cmp(ctx, shifted, IXS_CMP_GE, zero),
                                  ixs_cmp(ctx, base, IXS_CMP_GE, zero)) ==
        IXS_CHECK_TRUE);
  CHECK(test_ixs_equivalent_facts(
            grid, ixs_cmp(ctx, shifted, IXS_CMP_LE, seven),
            ixs_cmp(ctx, base, IXS_CMP_LE, seven)) == IXS_CHECK_TRUE);
  CHECK(test_ixs_equivalent_facts(
            grid, ixs_cmp(ctx, shifted, IXS_CMP_GT, seven),
            ixs_cmp(ctx, base, IXS_CMP_GT, seven)) == IXS_CHECK_TRUE);

  CHECK(ixs_facts_assume_pred(crossing, base_grid));
  CHECK(ixs_facts_assume_pred(crossing, shift_lower));
  CHECK(
      ixs_facts_assume_pred(crossing, ixs_cmp(ctx, shift, IXS_CMP_LE, eight)));
  CHECK(test_ixs_equivalent_facts(
            crossing, ixs_cmp(ctx, shifted, IXS_CMP_LT, zero),
            ixs_cmp(ctx, base, IXS_CMP_LT, zero)) != IXS_CHECK_TRUE);

  CHECK(ixs_facts_assume_pred(ungridded, shift_lower));
  CHECK(ixs_facts_assume_pred(ungridded, shift_upper));
  CHECK(test_ixs_equivalent_facts(
            ungridded, ixs_cmp(ctx, shifted, IXS_CMP_LT, zero),
            ixs_cmp(ctx, base, IXS_CMP_LT, zero)) != IXS_CHECK_TRUE);
  CHECK(test_ixs_equivalent_facts(grid, ixs_cmp(ctx, shifted, IXS_CMP_LE, zero),
                                  ixs_cmp(ctx, base, IXS_CMP_LE, zero)) !=
        IXS_CHECK_TRUE);

  CHECK(ixs_facts_assume_pred(lower_only, base_grid));
  CHECK(ixs_facts_assume_pred(lower_only, shift_lower));
  CHECK(test_ixs_equivalent_facts(
            lower_only, ixs_cmp(ctx, shifted, IXS_CMP_LT, zero),
            ixs_cmp(ctx, base, IXS_CMP_LT, zero)) != IXS_CHECK_TRUE);
  CHECK(ixs_facts_assume_pred(upper_only, base_grid));
  CHECK(ixs_facts_assume_pred(upper_only, shift_upper));
  CHECK(test_ixs_equivalent_facts(
            upper_only, ixs_cmp(ctx, shifted, IXS_CMP_LT, zero),
            ixs_cmp(ctx, base, IXS_CMP_LT, zero)) != IXS_CHECK_TRUE);

  CHECK(
      ixs_facts_assume_pred(large_facts, ixs_cmp(ctx, large, IXS_CMP_GE,
                                                 ixs_int(ctx, INT64_MAX - 1))));
  CHECK(ixs_facts_assume_pred(
      large_facts, ixs_cmp(ctx, large, IXS_CMP_LE, ixs_int(ctx, INT64_MAX))));
  CHECK(ixs_facts_assume_pred(
      small_facts, ixs_cmp(ctx, small, IXS_CMP_GE, ixs_int(ctx, INT64_MIN))));
  CHECK(
      ixs_facts_assume_pred(small_facts, ixs_cmp(ctx, small, IXS_CMP_LE,
                                                 ixs_int(ctx, INT64_MIN + 1))));
  errors = ctx->nerrors;
  CHECK(test_ixs_equivalent_facts(
            large_facts, ixs_cmp(ctx, large_shifted, IXS_CMP_GT, zero),
            ixs_cmp(ctx, base, IXS_CMP_GT, zero)) != IXS_CHECK_TRUE);
  CHECK(test_ixs_equivalent_facts(
            small_facts, ixs_cmp(ctx, small_shifted, IXS_CMP_LT, zero),
            ixs_cmp(ctx, base, IXS_CMP_LT, zero)) != IXS_CHECK_TRUE);
...
static void test_public_total_equivalence(void) {
...

  {
    ...
    ixs_node *fact_backed_base =
        ixs_add(ctx, ixs_mul(ctx, a, u), ixs_mul(ctx, ixs_int(ctx, 16), v));
    ...
    ixs_node *scaled_bounded_next = ixs_mul(ctx, ixs_int(ctx, 8), bounded_next);
    ...
    CHECK(ixs_facts_assume_pred(bounded_shift,
    ...
    CHECK(ixs_facts_assume_pred(bounded_shift,
                                ixs_cmp(ctx, r, IXS_CMP_LT, ixs_int(ctx, 16))));
    ...
    CHECK(ixs_facts_assume_pred(may_cross,
                                ixs_cmp(ctx, ixs_mod(ctx, a, ixs_int(ctx, 16)),
                                        IXS_CMP_EQ, ixs_int(ctx, 0))));
    ...
    CHECK(ixs_facts_assume_pred(may_cross,
                                ixs_cmp(ctx, r, IXS_CMP_GE, ixs_int(ctx, 0))));
    CHECK(ixs_facts_assume_pred(may_cross,
                                ixs_cmp(ctx, r, IXS_CMP_LE, ixs_int(ctx, 16))));
    ...
    CHECK(ixs_facts_assume_pred(missing_alignment,
                                ixs_cmp(ctx, r, IXS_CMP_GE, ixs_int(ctx, 0))));
    CHECK(ixs_facts_assume_pred(missing_alignment,
                                ixs_cmp(ctx, r, IXS_CMP_LT, ixs_int(ctx, 16))));
    ...
    CHECK(ixs_facts_assume_pred(scaled_shift,
                                ixs_cmp(ctx, ixs_mod(ctx, a, ixs_int(ctx, 16)),
                                        IXS_CMP_EQ, ixs_int(ctx, 0))));
    ...
    CHECK(ixs_facts_assume_pred(scaled_may_cross,
                                ixs_cmp(ctx, ixs_mod(ctx, a, ixs_int(ctx, 16)),
                                        IXS_CMP_EQ, ixs_int(ctx, 0))));
    ...
    CHECK(ixs_facts_assume_pred(scaled_may_cross,
                                ixs_cmp(ctx, r, IXS_CMP_GE, ixs_int(ctx, 0))));
    CHECK(ixs_facts_assume_pred(scaled_may_cross,
                                ixs_cmp(ctx, r, IXS_CMP_LE, ixs_int(ctx, 8))));
    ...
    CHECK(ixs_facts_assume_pred(scaled_missing_base,
                                ixs_cmp(ctx, r, IXS_CMP_GE, ixs_int(ctx, 0))));
    CHECK(ixs_facts_assume_pred(scaled_missing_base,
                                ixs_cmp(ctx, r, IXS_CMP_LT, ixs_int(ctx, 8))));
    ...
    CHECK(ixs_facts_assume_pred(scaled_missing_denominator,
                                ixs_cmp(ctx, ixs_mod(ctx, a, ixs_int(ctx, 16)),
                                        IXS_CMP_EQ, ixs_int(ctx, 0))));
    ...
    {
      ixs_node *raw0 = ixs_sym(ctx, "pow2_wrap_raw0");
      ixs_node *raw1 = ixs_sym(ctx, "pow2_wrap_raw1");
      ixs_node *raw2 = ixs_sym(ctx, "pow2_wrap_raw2");
      ixs_node *raw3 = ixs_sym(ctx, "pow2_wrap_raw3");
      ixs_node *wrapped = ixs_sym(ctx, "pow2_wrap_value");
      ixs_node *two31 = ixs_int(ctx, INT64_C(2147483648));
      ixs_node *two32 = ixs_int(ctx, INT64_C(4294967296));
      ixs_node *sixteen = ixs_int(ctx, 16);
      ixs_node *wrap_zero = ixs_int(ctx, 0);
      ixs_node *signed_sum =
          ixs_sub(ctx,
                  ixs_mod(ctx,
                          ixs_add(ctx, two31,
                                  ixs_add(ctx, ixs_mul(ctx, raw0, raw1),
                                          ixs_mul(ctx, raw2, raw3))),
                          two32),
                  two31);
      ixs_node *canonical =
          ixs_mod(ctx, ixs_mul(ctx, ixs_int(ctx, 2), wrapped), two32);
      ixs_node *shifted = ixs_add(ctx, canonical, ixs_int(ctx, 15));
      ixs_node *reconstructed = ixs_mod(ctx, shifted, two32);
      ixs_facts *aligned_wrap = ixs_facts_create(ctx);
      ixs_facts *unaligned_wrap = ixs_facts_create(ctx);
      ixs_node *bad_last = ixs_add(ctx, ixs_mod(ctx, sixteen, ixs_int(ctx, 24)),
                                   ixs_int(ctx, 15));
      ixs_node *boundary_last = ixs_int(ctx, 16);

      assume_public_integer_range(aligned_wrap, raw0, INT32_MIN, INT32_MAX);
      assume_public_integer_range(aligned_wrap, raw1, INT32_MIN, INT32_MAX);
      assume_public_integer_range(aligned_wrap, raw2, INT32_MIN, INT32_MAX);
      assume_public_integer_range(aligned_wrap, raw3, INT32_MIN, INT32_MAX);
      assume_public_integer_range(aligned_wrap, wrapped, INT32_MIN, INT32_MAX);
      CHECK(ixs_facts_assume_pred(
          aligned_wrap,
          ixs_cmp(ctx, ixs_mod(ctx, raw1, sixteen), IXS_CMP_EQ, wrap_zero)));
      CHECK(ixs_facts_assume_pred(
          aligned_wrap,
          ixs_cmp(ctx, ixs_mod(ctx, raw3, sixteen), IXS_CMP_EQ, wrap_zero)));
      CHECK(ixs_facts_assume_pred(
          aligned_wrap, ixs_cmp(ctx, wrapped, IXS_CMP_EQ, signed_sum)));
      CHECK(test_ixs_check_facts(aligned_wrap, ixs_cmp(ctx, reconstructed,
                                                       IXS_CMP_EQ, shifted)) ==
            IXS_CHECK_TRUE);

      assume_public_integer_range(unaligned_wrap, raw0, 0, 0);
      assume_public_integer_range(unaligned_wrap, raw1, 16, 16);
      assume_public_integer_range(unaligned_wrap, raw2, 1, 1);
      assume_public_integer_range(unaligned_wrap, raw3, -1, -1);
      assume_public_integer_range(unaligned_wrap, wrapped, -1, -1);
      CHECK(ixs_facts_assume_pred(
          unaligned_wrap,
          ixs_cmp(ctx, ixs_mod(ctx, raw1, sixteen), IXS_CMP_EQ, wrap_zero)));
      CHECK(ixs_facts_assume_pred(
          unaligned_wrap, ixs_cmp(ctx, wrapped, IXS_CMP_EQ, signed_sum)));
      CHECK(test_ixs_check_facts(unaligned_wrap,
                                 ixs_cmp(ctx, reconstructed, IXS_CMP_EQ,
                                         shifted)) == IXS_CHECK_FALSE);

      CHECK(test_ixs_equivalent_facts(empty,
                                      ixs_mod(ctx, bad_last, ixs_int(ctx, 24)),
                                      bad_last) == IXS_CHECK_FALSE);
      CHECK(test_ixs_equivalent_facts(empty,
                                      ixs_mod(ctx, boundary_last, sixteen),
                                      boundary_last) == IXS_CHECK_FALSE);
    }
  }
...
static void test_generic_enclosed_radix_chain(void) {
  ...
  ixs_node *bit1 = ixs_mod(
      ctx, ixs_floor(ctx, ixs_div(ctx, ixs_mod(ctx, x, sixty_four), two)), two);
  ixs_node *bit2 = ixs_mod(
      ctx, ixs_floor(ctx, ixs_div(ctx, ixs_mod(ctx, x, sixty_four), four)),
      two);
  ixs_node *partition =
      ixs_add(ctx, low,
              ixs_add(ctx, ixs_mul(ctx, two, bit1), ixs_mul(ctx, four, bit2)));
  ixs_node *direct = ixs_mod(ctx, x, eight);
  ixs_node *bad_bit1 = ixs_mod(
      ctx, ixs_floor(ctx, ixs_div(ctx, ixs_mod(ctx, x, six), two)), two);
  ixs_node *bad_partition = ixs_add(ctx, low, ixs_mul(ctx, two, bad_bit1));
  ...
  ixs_node *fractional_high = ixs_mod(
      ctx, ixs_floor(ctx, ixs_div(ctx, ixs_mod(ctx, fractional, eight), two)),
      two);
  ...
  ixs_node *scaled_partition = ixs_mul(ctx, ixs_int(ctx, 1056), partition);
  ixs_node *digit_64 = ixs_mod(
      ctx, ixs_floor(ctx, ixs_div(ctx, ixs_mod(ctx, x, sixty_four), eight)),
      two);
  ...
  ixs_node *full_partition =
      ixs_add(ctx, partition, ixs_mul(ctx, eight, digit_64));
  ixs_node *compact_partition =
      ixs_add(ctx, direct, ixs_mul(ctx, eight, compact_digit));
  ixs_node *other_direct = ixs_mod(ctx, other, ixs_int(ctx, 16));
  ixs_node *wrong_scaled_partition = ixs_mul(ctx, two, full_partition);
  ixs_node *extra_partition =
      ixs_add(ctx, full_partition, ixs_mul(ctx, ixs_int(ctx, 16), low));
  ixs_node *missing_partition = ixs_add(
      ctx, low,
      ixs_add(ctx, ixs_mul(ctx, two, bit1), ixs_mul(ctx, eight, digit_64)));
  ixs_node *duplicate_partition =
      ixs_add(ctx, full_partition, ixs_mul(ctx, two, ixs_mod(ctx, other, two)));
  ...
  CHECK(ctx && x && two && four && six && eight && sixty_four && low && bit1 &&
        bit2 && partition && direct && bad_bit1 && bad_partition &&
        bad_direct && fractional && fractional_low && fractional_high &&
  ...
  CHECK(ixs_facts_assume_pred(integer,
                              ixs_cmp(ctx, x, IXS_CMP_EQ, ixs_floor(ctx, x))));
  CHECK(ixs_facts_assume_pred(at_one,
  ...
  CHECK(test_ixs_equivalent_facts(at_one, extra_partition, full_partition) ==
        IXS_CHECK_FALSE);
  CHECK(test_ixs_equivalent_facts(at_six, missing_partition, full_partition) ==
        IXS_CHECK_FALSE);
...
static void test_generic_mod_reconstruction_from_quotient_fact(void) {
  ixs_ctx *ctx = ixs_ctx_create();
  ixs_node *x = ixs_sym(ctx, "mod_reconstruct_x");
  ixs_node *q = ixs_sym(ctx, "mod_reconstruct_q");
  ixs_node *eight = ixs_int(ctx, 8);
  ixs_node *quotient = ixs_floor(ctx, ixs_div(ctx, x, eight));
  ixs_node *reconstructed = ixs_sub(ctx, x, ixs_mul(ctx, eight, q));
  ixs_node *residue = ixs_mod(ctx, x, eight);
  ixs_facts *facts = ixs_facts_create(ctx);
  ixs_facts *integer_only = ixs_facts_create(ctx);

  CHECK(ctx && x && q && eight && quotient && reconstructed && residue &&
        facts && integer_only);
  CHECK(ixs_facts_assume_pred(facts,
                              ixs_cmp(ctx, x, IXS_CMP_EQ, ixs_floor(ctx, x))));
  CHECK(ixs_facts_assume_pred(facts, ixs_cmp(ctx, quotient, IXS_CMP_EQ, q)));
  CHECK(test_ixs_equivalent_facts(facts, reconstructed, residue) ==
        IXS_CHECK_TRUE);
  CHECK(test_ixs_check_predicate_facts(
            facts, ixs_cmp(ctx, reconstructed, IXS_CMP_EQ, residue)) ==
        IXS_CHECK_TRUE);

  CHECK(ixs_facts_assume_pred(integer_only,
                              ixs_cmp(ctx, x, IXS_CMP_EQ, ixs_floor(ctx, x))));
  CHECK(test_ixs_equivalent_facts(integer_only, reconstructed, residue) ==
        IXS_CHECK_UNKNOWN);
...
static void test_public_finite_difference_and_additive_split(void) {
  ...
  /* Predicates are scalar 0/1 expressions on algebra-query surfaces. */
...
static void test_public_exact_divide_fact_scaled_xor_quotient(void) {
  ...
  static const char dividend_text[] = "xor(2*Mod(x, 4), 6*Mod(y, 4))";
  static const char expected_text[] = "xor(Mod(x, 4), 3*Mod(y, 4))";
  ixs_ctx *ctx = ixs_ctx_create();
  ixs_node *dividend =
  ...
  CHECK(dividend && expected && origin && dividend_with_origin &&
  ...
  for (budget = first_fault_budget; budget <= first_success_budget; budget++) {
    ...
    CHECK(oom_dividend && oom_expected && oom_origin &&
...
static void test_public_defined_extrema_totalized_trunc_remainder(void) {
  ixs_ctx *ctx = ixs_ctx_create();
  ixs_node *numerator = ixs_sym(ctx, "totalized_numerator");
  ixs_node *divisor = ixs_sym(ctx, "totalized_divisor");
  ixs_node *extrema_args[3] = {
      ixs_int(ctx, 1),
      divisor,
      ixs_mul(ctx, ixs_int(ctx, -1), divisor),
  };
  ixs_node *magnitude = ixs_max_many(ctx, 3, extrema_args);
  ixs_node *quotient = ixs_trunc(ctx, ixs_div(ctx, numerator, magnitude));
  ixs_node *remainder =
      ixs_sub(ctx, numerator, ixs_mul(ctx, magnitude, quotient));
  ixs_node *scaled = ixs_mul(ctx, ixs_int(ctx, 16), remainder);
  ixs_facts *facts = ixs_facts_create(ctx);

  CHECK(facts != NULL);

  CHECK(ixs_check(ctx, ixs_cmp(ctx, magnitude, IXS_CMP_GE, ixs_int(ctx, 1)),
                  NULL, 0) == IXS_CHECK_TRUE);
  CHECK(ixs_check_defined(ctx, quotient, NULL, 0) == IXS_CHECK_TRUE);
  CHECK(ixs_check_defined(ctx, remainder, NULL, 0) == IXS_CHECK_TRUE);
  CHECK(ixs_check_defined(ctx, scaled, NULL, 0) == IXS_CHECK_TRUE);
  CHECK(ixs_check_integer_valued(ctx, remainder, NULL, 0) == IXS_CHECK_TRUE);
  CHECK(ixs_check_integer_valued(ctx, scaled, NULL, 0) == IXS_CHECK_TRUE);
  CHECK(test_ixs_check_facts(facts, ixs_cmp(ctx, scaled, IXS_CMP_EQ, scaled)) ==
        IXS_CHECK_TRUE);
  CHECK(test_ixs_check_facts(
            facts, ixs_cmp(ctx, scaled, IXS_CMP_EQ, ixs_floor(ctx, scaled))) ==
        IXS_CHECK_TRUE);
...
int main(void) {
  ...
  test_bounds_divisibility_through_mod();
  ...
  test_public_euclidean_division_identities();
  ...
  test_public_fact_integrality_rejects_modular_noninteger_factor();
  ...
  test_generic_piecewise_scalar_equivalence_by_partitions();
  test_generic_piecewise_scalar_equivalence_rejects_incomplete_proof();
  ...
  test_public_equivalence_ordered_bounded_delta();
  ...
  test_public_defined_extrema_totalized_trunc_remainder();
