...
static void test_public_integer_range(void) {
  ...
  CHECK(!ixs_integer_range(ctx, ixs_div(ctx, x, ixs_int(ctx, 2)), bounded, 2,
                           &result));
  ...
  CHECK(ixs_facts_assume_range(one_sided, x, &input));
  CHECK(ixs_integer_range_facts(one_sided, x, &result));
  CHECK(result.has_lower && result.lower == 1);
  ...
  input.lower_q = 1;
  CHECK(ixs_facts_assume_range(overflow, x, &input));
  CHECK(ixs_facts_assume_pred(overflow, x_mod_four));
  CHECK(ixs_integer_range_facts(overflow, x, &result));
  ...
  input.upper_q = 1;
  CHECK(ixs_facts_assume_range(contradictory, x, &input));
  CHECK(ixs_facts_assume_pred(contradictory, x_mod_four));
...
static void test_public_dynamic_modular_projection_difference(void) {
  ...
  CHECK(ixs_equivalent_facts(vector, value8,
                             ixs_add(ctx, value0, ixs_int(ctx, 8))) ==
        IXS_CHECK_UNKNOWN);
  CHECK(ixs_equivalent_facts(vector, value0,
                             ixs_sub(ctx, value8, ixs_int(ctx, 8))) ==
        IXS_CHECK_UNKNOWN);
  ...
  CHECK(!ixs_constant_difference_facts(zero_denominator, direct1, direct0,
                                       &delta));
  CHECK(ixs_equivalent_facts(zero_denominator, direct1,
                             ixs_add(ctx, direct0, ixs_int(ctx, 1))) ==
        IXS_CHECK_UNKNOWN);
  ...
  CHECK(!ixs_constant_difference_facts(negative_denominator, direct1, direct0,
                                       &delta));
  CHECK(ixs_equivalent_facts(negative_denominator, direct1,
                             ixs_add(ctx, direct0, ixs_int(ctx, 1))) ==
        IXS_CHECK_UNKNOWN);
...
static void test_public_modular_projection_residual_oom(void) {
  ixs_ctx *ctx = ixs_ctx_create();
  ixs_node *x = ixs_sym(ctx, "modular_projection_oom_x");
  ixs_node *y = ixs_sym(ctx, "modular_projection_oom_y");
  ixs_node *z = ixs_sym(ctx, "modular_projection_oom_z");
  ixs_node *wrapped0 = test_signed_wrap(ctx, x, 8, 16);
  ixs_node *wrapped1 =
      test_signed_wrap(ctx, ixs_add(ctx, x, ixs_int(ctx, 1)), 8, 16);
  ...
  /* The common proof prefix consumes 299 scratch allocations. The next one
   * builds the nonempty residual after removing the matched Mod pair. */
  ixs_arena_set_fail_after(ixs_test_scratch(ctx), 299);
...
static void test_public_constant_difference_wide_nonmod_unknown(void) {
  enum { TERM_COUNT = 256 };
  ixs_ctx *ctx = ixs_ctx_create();
  ixs_facts *facts = ixs_facts_create(ctx);
  ...
  for (i = 0; i < TERM_COUNT; i++) {
    char lhs_name[48];
    char rhs_name[48];
    snprintf(lhs_name, sizeof(lhs_name), "wide_nonmod_lhs_%u", i);
    snprintf(rhs_name, sizeof(rhs_name), "wide_nonmod_rhs_%u", i);
    lhs = ixs_add(ctx, lhs, ixs_sym(ctx, lhs_name));
    rhs = ixs_add(ctx, rhs, ixs_sym(ctx, rhs_name));
  }
  CHECK(!ixs_constant_difference_facts(facts, lhs, rhs, &delta));
  CHECK(!ixs_constant_difference_facts(facts, rhs, lhs, &delta));
...
static void test_public_defined_range_facts_do_not_close_domain(void) {
  ixs_ctx *ctx = ixs_ctx_create();
...
int main(void) {
  ...
  test_public_modular_projection_residual_oom();
  test_public_constant_difference_wide_nonmod_unknown();
  ...
  test_public_defined_range_facts_do_not_close_domain();
