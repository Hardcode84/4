...
static bool add_contains_term(const ixs_node *expr, const ixs_node *term) {
  uint32_t i;
  if (expr == term)
    return true;
  if (!expr || ixs_node_tag(expr) != IXS_ADD)
    return false;
  for (i = 0; i < ixs_node_add_nterms(expr); i++)
    if (ixs_node_add_term(expr, i) == term)
      return true;
  return false;
}
...
static void test_scaled_add_constant_cancellation(void) {
  ixs_ctx *ctx = get_ctx();
  ixs_node *x = ixs_sym(ctx, "scaled_add_cancellation_x");
  ixs_node *scale = ixs_int(ctx, INT64_C(4294967296));
  ixs_node *inner = ixs_add(ctx, ixs_int(ctx, INT64_C(2147483648)), x);
  ixs_node *scaled = ixs_mul(ctx, scale, inner);
  ixs_node *result = ixs_add(ctx, ixs_int(ctx, INT64_MIN), scaled);

  /* The product of the scale and inner constant is 2^63, but it cancels the
   * outer INT64_MIN exactly before the flattened ADD is materialized. */
  CHECK(result == ixs_mul(ctx, scale, x));

  {
    ixs_node *negative_scale = ixs_int(ctx, INT64_C(-4294967296));
    ixs_node *negative_inner =
        ixs_add(ctx, ixs_int(ctx, INT64_C(-2147483648)), x);
    ixs_node *negative_result =
        ixs_add(ctx, ixs_int(ctx, INT64_MIN),
                ixs_mul(ctx, negative_scale, negative_inner));
    CHECK(negative_result == ixs_mul(ctx, negative_scale, x));
  }

  {
    ixs_node *rational_scale =
        ixs_div(ctx, ixs_int(ctx, INT64_C(4294967296)), ixs_int(ctx, 3));
    ixs_node *rational_inner =
        ixs_add(ctx, ixs_int(ctx, INT64_C(6442450944)), x);
    ixs_node *rational_result =
        ixs_add(ctx, ixs_int(ctx, INT64_MIN),
                ixs_mul(ctx, rational_scale, rational_inner));
    CHECK(rational_result == ixs_mul(ctx, rational_scale, x));
  }

  {
    ixs_node *raw = ixs_sym(ctx, "scaled_add_bounded_raw");
    ixs_node *modulus = ixs_int(ctx, INT64_C(4294967296));
    ixs_node *quotient =
        ixs_floor(ctx, ixs_div(ctx, raw, ixs_int(ctx, INT64_C(1073741824))));
    ixs_node *biased =
        ixs_add(ctx, ixs_int(ctx, INT64_C(2147483648)), quotient);
    ixs_node *wrapped = ixs_mod(ctx, biased, modulus);
    ixs_node *remainder =
        ixs_mod(ctx, ixs_mul(ctx, ixs_int(ctx, 4), raw), modulus);
    size_t errors = ixs_ctx_nerrors(ctx);
    ixs_node *expr = ixs_add(
        ctx, remainder,
        ixs_add(ctx, ixs_int(ctx, INT64_MIN), ixs_mul(ctx, scale, wrapped)));
    ixs_node *assumptions[] = {
        ixs_cmp(ctx, raw, IXS_CMP_GE, ixs_int(ctx, INT64_C(-2147483648))),
        ixs_cmp(ctx, raw, IXS_CMP_LE, ixs_int(ctx, INT64_C(2147483647))),
    };

    /* Bounds remove the inner fixed-width Mod, then exact constant
     * cancellation exposes the remaining floor/Mod algebra. */
    CHECK(expr && !ixs_is_error(expr));
    CHECK(ixs_ctx_nerrors(ctx) == errors);
    result = ixs_simplify(ctx, expr, assumptions, 2);
    CHECK(result == ixs_mul(ctx, ixs_int(ctx, 4), raw));
    CHECK(ixs_ctx_nerrors(ctx) == errors);

    {
      ...
      result = (ixs_node *)test_ixs_simplify_facts(facts, expr);
      CHECK(result == ixs_mul(ctx, ixs_int(ctx, 4), raw));
      CHECK(ixs_ctx_nerrors(ctx) == errors);
    }

    {
      ixs_node *narrow_assumptions[] = {
          ixs_cmp(ctx, raw, IXS_CMP_GE, ixs_int(ctx, 0)),
          ixs_cmp(ctx, raw, IXS_CMP_LE, ixs_int(ctx, 31)),
      };
      result = ixs_simplify(ctx, expr, narrow_assumptions, 2);
      CHECK(result == ixs_mul(ctx, ixs_int(ctx, 4), raw));
      CHECK(ixs_ctx_nerrors(ctx) == errors);
    }
  }
...
#ifdef IXS_TEST_INTERNAL
  {
    ...
    ixs_node *overflow_inner =
        ixs_add(ctx, ixs_int(ctx, 1), ixs_sym(ctx, "scaled_add_overflow_x"));
    ixs_node *overflow_scaled = ixs_mul(ctx, ixs_int(ctx, 2), overflow_inner);
    ixs_node *overflow_result;
    size_t errors = ixs_ctx_nerrors(ctx);
    bool unrepresentable = false;

    CHECK(ixs_session_bind(&binding, IXS_TEST_SESSION(ctx)) == ctx);
    overflow_result = simp_try_add(ctx, ixs_int(ctx, INT64_MAX),
                                   overflow_scaled, &unrepresentable);
...
#ifdef IXS_TEST_INTERNAL
static void test_exact_quotient_common_denominator(void) {
  ixs_ctx *ctx = ctx_create_or_die();
  ixs_session_binding binding;
  ixs_node *x = ixs_sym(ctx, "quotient_common_x");
  ixs_node *y = ixs_sym(ctx, "quotient_common_y");
  ixs_node *mixed = ixs_add(ctx, ixs_div(ctx, x, ixs_int(ctx, 16)),
                            ixs_div(ctx, y, ixs_int(ctx, 64)));
  ixs_node *overflow = ixs_add(ctx, ixs_div(ctx, x, ixs_int(ctx, INT64_MAX)),
                               ixs_div(ctx, y, ixs_int(ctx, INT64_MAX - 1)));
  ixs_node *numerator;
  ixs_node *denominator;
  ixs_node *expected = ixs_add(ctx, ixs_mul(ctx, ixs_int(ctx, 4), x), y);
  ixs_quotient_parts_status mixed_status;

  CHECK(ixs_session_bind(&binding, IXS_TEST_SESSION(ctx)) == ctx);
  mixed_status =
      simp_decompose_exact_quotient(ctx, mixed, &numerator, &denominator);
  CHECK(mixed_status == IXS_QUOTIENT_PARTS_MATCH);
  CHECK(numerator == expected);
  CHECK(denominator == ixs_int(ctx, 64));
  CHECK(
      simp_decompose_exact_quotient(ctx, overflow, &numerator, &denominator) ==
...
static void test_boolean_piecewise(void) {
...

  {
    ...
    CHECK(ixs_cmp(ctx, nested, IXS_CMP_NE, zero) == selector);
  }

  {
    ixs_node *inverted_values[] = {zero, one};
    ixs_node *inverted = ixs_pw(ctx, 2, inverted_values, conditions);
    CHECK(inverted == ixs_not(ctx, condition));
    CHECK(ixs_cmp(ctx, inverted, IXS_CMP_NE, zero) == ixs_not(ctx, condition));
  }

  {
    ixs_node *non_boolean_values[] = {ixs_int(ctx, 2), zero};
    ixs_node *inverted_carrier_values[] = {zero, ixs_int(ctx, -7)};
    ...
    ixs_node *inverted_carrier =
        ixs_pw(ctx, 2, inverted_carrier_values, conditions);
    ixs_node *partial = ixs_pw(ctx, 1, partial_values, partial_conditions);
    ixs_node *numeric_truth = ixs_cmp(ctx, x, IXS_CMP_NE, zero);
    ixs_node *dynamic_values[] = {x, zero};
    ixs_node *dynamic_carrier = ixs_pw(ctx, 2, dynamic_values, conditions);
    ixs_node *second_condition = ixs_cmp(ctx, x, IXS_CMP_GT, ixs_int(ctx, -4));
    ixs_node *non_total_conditions[] = {condition, second_condition};
    ixs_node *non_total_carrier =
        ixs_pw(ctx, 2, non_boolean_values, non_total_conditions);
    ixs_node *unfolded;

    CHECK(ixs_node_tag(non_boolean) == IXS_PIECEWISE);
    CHECK(ixs_cmp(ctx, non_boolean, IXS_CMP_NE, zero) == condition);
    CHECK(ixs_cmp(ctx, non_boolean, IXS_CMP_EQ, zero) ==
          ixs_not(ctx, condition));
    CHECK(ixs_cmp(ctx, zero, IXS_CMP_NE, non_boolean) == condition);
    CHECK(ixs_cmp(ctx, zero, IXS_CMP_EQ, non_boolean) ==
          ixs_not(ctx, condition));
    CHECK(ixs_cmp(ctx, inverted_carrier, IXS_CMP_NE, zero) ==
          ixs_not(ctx, condition));
    CHECK(ixs_cmp(ctx, inverted_carrier, IXS_CMP_EQ, zero) == condition);
    CHECK(ixs_node_tag(partial) == IXS_PIECEWISE);
    CHECK(ixs_node_tag(numeric_truth) == IXS_CMP);
    CHECK(numeric_truth != x);
    unfolded = ixs_cmp(ctx, partial, IXS_CMP_NE, zero);
    CHECK(ixs_node_tag(unfolded) == IXS_PIECEWISE);
    unfolded = ixs_cmp(ctx, dynamic_carrier, IXS_CMP_NE, zero);
...
static void test_xor_known_bit_simplification(void) {
...

  {
    ...
    ixs_node *aligned = ixs_add(ctx, four_b, eight_c);
    ...
    CHECK(ixs_simplify(
              ctx,
              ixs_xor(ctx, aligned, ixs_add(ctx, aligned, ixs_int(ctx, 1))),
              assumes, 6) == ixs_int(ctx, 1));
    CHECK(ixs_simplify(
              ctx,
              ixs_xor(ctx, aligned, ixs_add(ctx, aligned, ixs_int(ctx, 2))),
              assumes, 6) == ixs_int(ctx, 2));
    CHECK(ixs_node_tag(
              ixs_simplify(ctx,
                           ixs_xor(ctx, ixs_add(ctx, aligned, ixs_int(ctx, 2)),
                                   ixs_add(ctx, aligned, ixs_int(ctx, 4))),
                           assumes, 6)) == IXS_XOR);
...
static unsigned substitution_legacy_memo_slot(const ixs_node *node) {
  uint32_t hash = ixs_node_hash(node);
  return (unsigned)((hash ^ (hash >> 8)) & 255u);
}

static ixs_node *
substitution_colliding_piecewise(ixs_ctx *ctx, ixs_node *first,
                                 ixs_node *second, uint32_t level,
                                 ixs_node **selected_condition) {
  uint32_t trial;
  for (trial = 0; trial < 100000u; trial++) {
    char name[64];
    ixs_node *values[2] = {first, second};
    ixs_node *conditions[2];
    ixs_node *candidate;
    (void)snprintf(name, sizeof(name), "subs_collision_%" PRIu32 "_%" PRIu32,
                   level, trial);
    conditions[0] =
        ixs_cmp(ctx, ixs_sym(ctx, name), IXS_CMP_NE, ixs_int(ctx, 0));
    conditions[1] = ixs_true(ctx);
    candidate = ixs_pw(ctx, 2, values, conditions);
    if (!candidate)
      return NULL;
    if (substitution_legacy_memo_slot(candidate) == 0u) {
...
static void test_substitution_deep_iterative(void) {
  enum { DEPTH = 65536 };
  ixs_ctx *ctx = ctx_create_or_die();
  ixs_node *x = ixs_sym(ctx, "subs_deep_x");
  ixs_node *y = ixs_sym(ctx, "subs_deep_y");
  ixs_node *source = x;
  ixs_node *expected = y;
  ...
  for (i = 0; i < DEPTH; i++) {
    if ((i & 1u) == 0) {
      source = ixs_max(ctx, source, zero);
      expected = ixs_max(ctx, expected, zero);
    } else {
      source = ixs_min(ctx, source, one);
      expected = ixs_min(ctx, expected, one);
    }
  }
  CHECK(source != NULL);
  CHECK(expected != NULL);
  CHECK(ixs_subs(ctx, source, x, y) == expected);
  ixs_ctx_destroy(ctx);
}

static void test_substitution_collision_dag(void) {
  enum { DEPTH = 38 };
  ixs_ctx *ctx = ctx_create_or_die();
  ixs_node *x = ixs_sym(ctx, "subs_collision_x");
  ixs_node *y = ixs_sym(ctx, "subs_collision_y");
  ixs_node *source_previous2 = ixs_max(ctx, x, ixs_int(ctx, 0));
  ixs_node *source_previous1 = ixs_min(ctx, x, ixs_int(ctx, 1));
  ixs_node *expected_previous2 = ixs_max(ctx, y, ixs_int(ctx, 0));
  ixs_node *expected_previous1 = ixs_min(ctx, y, ixs_int(ctx, 1));
  uint32_t level;

  for (level = 2; level <= DEPTH; level++) {
    ixs_node *condition = NULL;
    ixs_node *source = substitution_colliding_piecewise(
        ctx, source_previous1, source_previous2, level, &condition);
    ixs_node *expected_values[2] = {expected_previous1, expected_previous2};
    ixs_node *expected_conditions[2] = {condition, ixs_true(ctx)};
    ixs_node *expected;
    CHECK(source != NULL);
    CHECK(condition != NULL);
    if (!source || !condition)
      break;
    CHECK(substitution_legacy_memo_slot(source) == 0u);
    expected = ixs_pw(ctx, 2, expected_values, expected_conditions);
    CHECK(expected != NULL);
    source_previous2 = source_previous1;
    source_previous1 = source;
    expected_previous2 = expected_previous1;
    expected_previous1 = expected;
  }
  CHECK(level == DEPTH + 1u);
  CHECK(ixs_subs(ctx, source_previous1, x, y) == expected_previous1);
  ixs_ctx_destroy(ctx);
}
...
static void test_substitution_failure_retry(void) {
  ...
  ixs_node *x = ixs_sym(ctx, "subs_failure_x");
  ixs_node *y = ixs_sym(ctx, "subs_failure_y");
  ixs_node *source = x;
  ixs_node *expected = y;
  ixs_node *zero = ixs_int(ctx, 0);
  ixs_node *one = ixs_int(ctx, 1);
  ...
  size_t allocations;
  size_t budget;
  size_t scratch_failures = 0;
  size_t store_failures = 0;
  size_t store_successes = 0;
  uint32_t i;

  for (i = 0; i < SCRATCH_DEPTH; i++) {
    if ((i & 1u) == 0) {
      source = ixs_max(ctx, source, zero);
      expected = ixs_max(ctx, expected, zero);
    } else {
  ...
  for (budget = 0; budget < allocations; budget++) {
    ...
    failed = ixs_subs(ctx, source, x, y);
    ixs_arena_set_fail_after(scratch, IXS_ARENA_FAILURE_DISABLED);
    CHECK(failed == NULL);
    if (!failed)
      scratch_failures++;
  ...
  CHECK(ixs_subs(ctx, source, x, y) == expected);
  ixs_arena_set_fail_after(scratch, IXS_ARENA_FAILURE_DISABLED);
  CHECK(scratch_failures == allocations);

  source = x;
  for (i = 0; i < STORE_DEPTH; i++)
    source =
        (i & 1u) == 0 ? ixs_max(ctx, source, zero) : ixs_min(ctx, source, one);

  for (budget = 0; budget < 256u && store_successes < 4u; budget++) {
    char name[64];
    ixs_node *replacement;
    ixs_node *result;
    (void)snprintf(name, sizeof(name), "subs_store_retry_%zu", budget);
    replacement = ixs_sym(ctx, name);
    CHECK(replacement != NULL);
    ixs_arena_set_fail_after(&ctx->arena, budget);
    result = ixs_subs(ctx, source, x, replacement);
    ...
    if (result)
      CHECK(ixs_subs(ctx, result, replacement, x) == source);
  }
  CHECK(store_failures > 0);
...
static void test_floor_shift_xor(void) {
  ...
  /* Toggling a low field in a carry-free bit reconstruction cannot change
   * the quotient formed from the disjoint high field. */
  {
    ...
    ixs_node *bit5 =
        ixs_mod(ctx, ixs_floor(ctx, ixs_div(ctx, x, ixs_int(ctx, 32))),
                ixs_int(ctx, 2));
    ixs_node *bit6 =
        ixs_mod(ctx, ixs_floor(ctx, ixs_div(ctx, x, ixs_int(ctx, 64))),
                ixs_int(ctx, 2));
    ixs_node *terms[4] = {bit0, ixs_mul(ctx, ixs_int(ctx, 2), bit1),
                          ixs_mul(ctx, ixs_int(ctx, 32), bit5),
                          ixs_mul(ctx, ixs_int(ctx, 64), bit6)};
    ixs_node *toggled_terms[4] = {bit0, ixs_xor(ctx, ixs_int(ctx, 2), terms[1]),
                                  terms[2], terms[3]};
    ixs_node *high = ixs_floor(
        ctx, ixs_div(ctx, ixs_xor_many(ctx, 4, terms), ixs_int(ctx, 64)));
    ixs_node *toggled_high =
        ixs_floor(ctx, ixs_div(ctx, ixs_xor_many(ctx, 4, toggled_terms),
                               ixs_int(ctx, 64)));
    ixs_node *canonical = ixs_simplify(ctx, bit6, assumptions, 2);
    ixs_node *same = ixs_cmp(ctx, high, IXS_CMP_EQ, toggled_high);

    CHECK(ixs_simplify(ctx, high, assumptions, 2) == canonical);
    CHECK(ixs_simplify(ctx, toggled_high, assumptions, 2) == canonical);
    CHECK(ixs_simplify(ctx, same, assumptions, 2) == ixs_int(ctx, 1));
    {
      ixs_facts *facts = ixs_facts_create(ctx);
      CHECK(facts != NULL);
      CHECK(ixs_facts_assume_preds(facts, assumptions, 2));
      CHECK(test_ixs_check_predicate_facts(facts, same) == IXS_CHECK_TRUE);
    }

    {
      uint64_t before = stat_count(ctx, "floor_shift_xor");
      ixs_node *non_power =
          ixs_floor(ctx, ixs_div(ctx, ixs_xor_many(ctx, 4, toggled_terms),
                                 ixs_int(ctx, 48)));
      CHECK(ixs_simplify(ctx, non_power, assumptions, 2) != NULL);
      CHECK(stat_count(ctx, "floor_shift_xor") == before);
    }

    {
      uint64_t before = stat_count(ctx, "floor_shift_xor");
      CHECK(ixs_simplify(ctx, shifted, NULL, 0) != NULL);
      CHECK(stat_count(ctx, "floor_shift_xor") == before);
    }
  }
}
...
static void test_mod_bitwise_projection(void) {
  ixs_ctx *ctx = get_ctx();
  ixs_node *x = ixs_sym(ctx, "mod_projection_x");
  ixs_node *y = ixs_sym(ctx, "mod_projection_y");
  ixs_node *dynamic_modulus = ixs_sym(ctx, "mod_projection_modulus");
  ixs_node *modulus = ixs_int(ctx, 8);
  ixs_node *assumptions[4] = {
      ixs_cmp(ctx, x, IXS_CMP_GE, ixs_int(ctx, 0)),
      ixs_cmp(ctx, x, IXS_CMP_LT, ixs_int(ctx, 32)),
      ixs_cmp(ctx, y, IXS_CMP_GE, ixs_int(ctx, 0)),
      ixs_cmp(ctx, y, IXS_CMP_LT, ixs_int(ctx, 32)),
  };
  ...
  CHECK(ixs_simplify(ctx, and_projection, assumptions, 4) == expected_and);
  CHECK(ixs_simplify(ctx, or_projection, assumptions, 4) == expected_or);

  {
    ixs_node *non_power_of_two =
        ixs_mod(ctx, ixs_and(ctx, x, y), ixs_int(ctx, 6));
    CHECK(ixs_simplify(ctx, non_power_of_two, assumptions, 4) ==
          non_power_of_two);
  }
  ...
  {
    ixs_node *partial = ixs_mod(ctx, x, dynamic_modulus);
    ixs_node *possibly_undefined =
        ixs_mod(ctx, ixs_or(ctx, partial, y), modulus);
    CHECK(ixs_simplify(ctx, possibly_undefined, assumptions, 4) ==
          possibly_undefined);
  }
}
...
static void test_fact_equality_definition_normalization(void) {
  ...
  ixs_node *batch[2] = {expr, source_y};
  ixs_facts *facts = ixs_facts_create(ctx);

  CHECK(ixs_facts_assume_preds(facts, definitions, 2));
...
static void test_mod_representative_definition_proof(void) {
  ixs_ctx *ctx = get_ctx();
  ixs_node *block = ixs_sym(ctx, "definition_unpack_block");
  ixs_node *item = ixs_sym(ctx, "definition_unpack_item");
  ixs_node *slot = ixs_sym(ctx, "definition_unpack_slot");
  ixs_node *mapped_item = ixs_sym(ctx, "definition_mapped_item");
  ixs_node *mapped_slot = ixs_sym(ctx, "definition_mapped_slot");
  ixs_node *item_value = ixs_mod(ctx, item, ixs_int(ctx, 64));
  ixs_node *slot_value = ixs_mod(ctx, slot, ixs_int(ctx, 8));
  ixs_node *predicates[8] = {
      ixs_cmp(ctx, block, IXS_CMP_GE, ixs_int(ctx, 0)),
      ixs_cmp(ctx, block, IXS_CMP_LT, ixs_int(ctx, 4)),
      ixs_cmp(ctx, item, IXS_CMP_GE, ixs_int(ctx, 0)),
      ixs_cmp(ctx, item, IXS_CMP_LT, ixs_int(ctx, 256)),
      ixs_cmp(ctx, slot, IXS_CMP_GE, ixs_int(ctx, 0)),
      ixs_cmp(ctx, slot, IXS_CMP_LT, ixs_int(ctx, 128)),
      ixs_cmp(ctx, mapped_item, IXS_CMP_EQ, item_value),
      ixs_cmp(ctx, mapped_slot, IXS_CMP_EQ, slot_value),
  };
  ...
  CHECK(test_ixs_check_facts(facts, goal) == IXS_CHECK_TRUE);
  CHECK(test_ixs_check_facts(facts, false_goal) == IXS_CHECK_FALSE);
  CHECK(test_ixs_check_facts(facts, unknown_goal) == IXS_CHECK_UNKNOWN);
  CHECK(test_ixs_check_divisible_facts(facts, difference, 256) ==
        IXS_CHECK_TRUE);
...
static void test_exact_divide_fact_piecewise(void) {
  ...
  ixs_node *total_condition = ixs_cmp(ctx, item, IXS_CMP_GE, ixs_int(ctx, 0));
  ixs_node *total_values[2] = {ixs_int(ctx, 2), ixs_int(ctx, 0)};
  ixs_node *total_conditions[2] = {total_condition, ixs_true(ctx)};
  ixs_node *total_piecewise = ixs_pw(ctx, 2, total_values, total_conditions);
  ixs_node *in_range =
...
static void test_mod_difference_congruence(void) {
  ...
  {
    enum { DECOYS = 300 };
    ixs_node *pair_x = ixs_sym(ctx, "pair_x");
    ixs_node *pair_y = ixs_sym(ctx, "pair_y");
    ixs_node *pair_modulus = ixs_int(ctx, 257);
    ixs_node *pair_x_mod = ixs_mod(ctx, pair_x, pair_modulus);
    ixs_node *pair_y_mod = ixs_mod(ctx, pair_y, pair_modulus);
    ixs_node *large = ixs_sub(ctx, pair_x_mod, pair_y_mod);
    ixs_node *target_relation =
        ixs_cmp(ctx, pair_x, IXS_CMP_EQ, ixs_add(ctx, pair_y, pair_modulus));
    ixs_node *large_result;
    uint32_t i;

    /* More than the former fixed pair budget precedes the cancellable pair
     * in canonical order.  Distinct moduli are indexed linearly and cannot
     * hide the complete congruence proof. */
    for (i = 0; i < DECOYS; i++) {
      char name[64];
      ixs_node *decoy;
      (void)snprintf(name, sizeof(name), "pair_decoy_%u", i);
      decoy = ixs_mod(ctx, ixs_sym(ctx, name), ixs_int(ctx, 1009 + i));
      CHECK(decoy != NULL);
      large = ixs_add(ctx, large, decoy);
      CHECK(large != NULL);
    }
    CHECK(target_relation != NULL);
    large_result = ixs_simplify(ctx, large, &target_relation, 1);
    CHECK(large_result && ixs_node_tag(large_result) == IXS_ADD);
    CHECK(ixs_node_add_nterms(large_result) == DECOYS);
    CHECK(!add_contains_term(large_result, pair_x_mod));
    CHECK(!add_contains_term(large_result, pair_y_mod));
  }
}
...
static void test_subs_power_overflow(void) {
  ...
  /* Exponent magnitude is not a folding cap. */
  {
    ixs_node *power = x;
    unsigned i;
    for (i = 1; i < 65u; i++)
      power = ixs_mul(ctx, power, x);
    r = ixs_subs(ctx, power, x, ixs_int(ctx, -1));
    CHECK(r == ixs_int(ctx, -1));
  }
}

#ifdef IXS_TEST_INTERNAL
static void test_try_subs_arithmetic_contract(void) {
  ixs_ctx *ctx = get_ctx();
  ixs_session_binding binding;
  ...
  CHECK(target && replacement && scaled && expr && power);
  CHECK(ixs_session_bind(&binding, IXS_TEST_SESSION(ctx)) == ctx);
  result = simp_try_subs(ctx, expr, target, replacement, &unrepresentable);
  CHECK(result == NULL);
  CHECK(unrepresentable);
  CHECK(ixs_ctx_nerrors(ctx) == errors);
...
/* Consecutive quotient/remainder decompositions compose for every positive
 * integer base: Mod(x,a) + a*Mod(floor(x/a),b) = Mod(x,a*b). */
...
static void test_complement_annihilation(void) {
  ...
  /* A direct predicate subsumes its total nested complement. */
  {
...
static void test_fact_mod_congruence_bounds(void) {
  ixs_ctx *ctx = get_ctx();
  ixs_node *a = ixs_sym(ctx, "mod_range_a");
  ixs_node *b = ixs_sym(ctx, "mod_range_b");
  ixs_node *selector = ixs_sym(ctx, "mod_range_selector");
  ixs_node *modulus = ixs_int(ctx, INT64_C(4294967296));
  ixs_node *limit = ixs_int(ctx, INT64_C(4294967295));
  ixs_node *selector_bit = ixs_mod(ctx, selector, ixs_int(ctx, 2));
  ixs_node *product = ixs_mul(ctx, a, selector_bit);
  ixs_node *dividend = ixs_mul(ctx, ixs_int(ctx, 2), ixs_add(ctx, b, product));
  ...
  ixs_node *aligned[] = {
      ixs_cmp(ctx, ixs_mod(ctx, a, ixs_int(ctx, 16)), IXS_CMP_EQ,
              ixs_int(ctx, 0)),
      ixs_cmp(ctx, ixs_mod(ctx, b, ixs_int(ctx, 256)), IXS_CMP_EQ,
              ixs_int(ctx, 0)),
  };
  ixs_node *under_aligned[] = {
      ixs_cmp(ctx, ixs_mod(ctx, a, ixs_int(ctx, 2)), IXS_CMP_EQ,
              ixs_int(ctx, 0)),
      ixs_cmp(ctx, ixs_mod(ctx, b, ixs_int(ctx, 2)), IXS_CMP_EQ,
              ixs_int(ctx, 0)),
  };
  ixs_node *result;

  /* Every term is divisible by eight through facts and product algebra, so
   * adding seven cannot cross the 32-bit remainder boundary. */
  CHECK(ixs_simplify(ctx, window, aligned, 2) == ixs_true(ctx));

  /* Divisibility by four does not justify the same seven-unit window. */
  result = ixs_simplify(ctx, window, under_aligned, 2);
  CHECK(result != ixs_true(ctx) && result != ixs_false(ctx));

  /* Nonzero congruence classes use the same remainder envelope. */
  {
    ixs_node *x = ixs_sym(ctx, "mod_range_x");
    ixs_node *y = ixs_sym(ctx, "mod_range_y");
    ixs_node *xy = ixs_mul(ctx, x, y);
    ixs_node *mod_xy = ixs_mod(ctx, xy, modulus);
    ixs_node *fits =
        ixs_cmp(ctx, ixs_add(ctx, mod_xy, ixs_int(ctx, 4)), IXS_CMP_LE, limit);
    ixs_node *does_not_fit =
        ixs_cmp(ctx, ixs_add(ctx, mod_xy, ixs_int(ctx, 5)), IXS_CMP_LE, limit);
    ixs_node *facts[] = {
        ixs_cmp(ctx, ixs_mod(ctx, x, ixs_int(ctx, 8)), IXS_CMP_EQ,
                ixs_int(ctx, 1)),
        ixs_cmp(ctx, ixs_mod(ctx, y, ixs_int(ctx, 8)), IXS_CMP_EQ,
                ixs_int(ctx, 3)),
    };

    CHECK(ixs_simplify(ctx, fits, facts, 2) == ixs_true(ctx));
    result = ixs_simplify(ctx, does_not_fit, facts, 2);
    CHECK(result != ixs_true(ctx) && result != ixs_false(ctx));
  }
}

static void test_affine_endpoint_refinement(void) {
  ixs_ctx *ctx = get_ctx();
  ixs_node *x = ixs_sym(ctx, "endpoint_digit_x");
  ixs_node *x_mod_64 = ixs_mod(ctx, x, ixs_int(ctx, 64));
  ixs_node *digits[6];
  ixs_node *sum = ixs_int(ctx, 0);
  ixs_node *predicates[] = {
      ixs_cmp(ctx, x, IXS_CMP_GE, ixs_int(ctx, 0)),
      ixs_cmp(ctx, x, IXS_CMP_LE, ixs_int(ctx, 511)),
      NULL,
  };
  ixs_facts *facts = ixs_facts_create(ctx);
  size_t i;
  ...
  for (i = 0; i < sizeof(digits) / sizeof(digits[0]); i++) {
    ...
    digits[i] = ixs_mod(
        ctx,
        i == 0 ? x_mod_64
               : ixs_floor(ctx, ixs_div(ctx, x_mod_64, ixs_int(ctx, place))),
        ixs_int(ctx, 2));
    sum = ixs_add(ctx, sum, ixs_mul(ctx, ixs_int(ctx, place), digits[i]));
  }
  predicates[2] = ixs_cmp(ctx, sum, IXS_CMP_EQ, ixs_int(ctx, 0));

  CHECK(ixs_facts_assume_preds(facts, predicates, 3));
  for (i = 0; i < sizeof(digits) / sizeof(digits[0]); i++) {
    ixs_check_result digit_result = test_ixs_check_facts(
        facts, ixs_cmp(ctx, digits[i], IXS_CMP_EQ, ixs_int(ctx, 0)));
    CHECK(digit_result == IXS_CHECK_TRUE);
  }
  {
    ixs_node *half = ixs_div(ctx, x, ixs_int(ctx, 2));
    ixs_node *transferred = ixs_mod(ctx, half, ixs_int(ctx, 2));
    ixs_node *unconstrained = ixs_mod(ctx, half, ixs_int(ctx, 8));

    /* The exact range follows the fact-simplified expression identity, but
     * does not overconstrain a different modulus on the same dividend. */
    CHECK(test_ixs_check_facts(
              facts, ixs_cmp(ctx, transferred, IXS_CMP_EQ, ixs_int(ctx, 0))) ==
          IXS_CHECK_TRUE);
    CHECK(test_ixs_check_facts(facts, ixs_cmp(ctx, unconstrained, IXS_CMP_EQ,
                                              ixs_int(ctx, 0))) ==
          IXS_CHECK_UNKNOWN);
  }

  /* Coefficient signs select opposite term endpoints at a sum endpoint. */
  {
    ixs_node *a = ixs_sym(ctx, "endpoint_interior_a");
    ixs_node *b = ixs_sym(ctx, "endpoint_interior_b");
    ixs_node *difference = ixs_sub(ctx, a, b);
    ixs_node *endpoint_predicates[] = {
        ixs_cmp(ctx, a, IXS_CMP_GE, ixs_int(ctx, 0)),
        ixs_cmp(ctx, a, IXS_CMP_LE, ixs_int(ctx, 1)),
        ixs_cmp(ctx, b, IXS_CMP_GE, ixs_int(ctx, 0)),
        ixs_cmp(ctx, b, IXS_CMP_LE, ixs_int(ctx, 1)),
        ixs_cmp(ctx, difference, IXS_CMP_EQ, ixs_int(ctx, -1)),
    };
    ixs_node *interior_predicates[] = {
        ixs_cmp(ctx, a, IXS_CMP_GE, ixs_int(ctx, 0)),
        ixs_cmp(ctx, a, IXS_CMP_LE, ixs_int(ctx, 1)),
        ixs_cmp(ctx, b, IXS_CMP_GE, ixs_int(ctx, 0)),
        ixs_cmp(ctx, b, IXS_CMP_LE, ixs_int(ctx, 1)),
        ixs_cmp(ctx, difference, IXS_CMP_EQ, ixs_int(ctx, 0)),
    };
    ixs_facts *endpoint = ixs_facts_create(ctx);
    ixs_facts *interior = ixs_facts_create(ctx);

    CHECK(ixs_facts_assume_preds(endpoint, endpoint_predicates, 5));
    CHECK(test_ixs_check_facts(endpoint,
                               ixs_cmp(ctx, a, IXS_CMP_EQ, ixs_int(ctx, 0))) ==
          IXS_CHECK_TRUE);
    CHECK(test_ixs_check_facts(endpoint,
                               ixs_cmp(ctx, b, IXS_CMP_EQ, ixs_int(ctx, 1))) ==
          IXS_CHECK_TRUE);

    /* An interior sum value does not constrain either term to an endpoint. */
    CHECK(ixs_facts_assume_preds(interior, interior_predicates, 5));
...
static void test_partitioned_affine_endpoint_refinement(void) {
  ixs_ctx *ctx = get_ctx();
  ixs_node *index = ixs_sym(ctx, "endpoint_partitioned_index");
  ixs_node *low_field = ixs_mod(ctx, index, ixs_int(ctx, 64));
  ixs_node *digits[7];
  ixs_node *key = ixs_int(ctx, 0);
  ixs_node *predicates[] = {
      ixs_cmp(ctx, index, IXS_CMP_GE, ixs_int(ctx, 0)),
      ixs_cmp(ctx, index, IXS_CMP_LE, ixs_int(ctx, 255)),
      NULL,
  };
  ixs_facts *facts = ixs_facts_create(ctx);
  size_t i;
  ...
  for (i = 0; i < 5; i++) {
    ...
    digits[i] = ixs_mod(
        ctx,
        i == 0 ? low_field
               : ixs_floor(ctx, ixs_div(ctx, low_field, ixs_int(ctx, divisor))),
        ixs_int(ctx, 2));
  }
  for (i = 0; i < 2; i++) {
    int64_t divisor = INT64_C(64) << i;
    digits[5 + i] =
        ixs_mod(ctx, ixs_floor(ctx, ixs_div(ctx, index, ixs_int(ctx, divisor))),
                ixs_int(ctx, 2));
  }
  for (i = 0; i < sizeof(digits) / sizeof(digits[0]); i++)
    key = ixs_add(ctx, key,
                  ixs_mul(ctx, ixs_int(ctx, INT64_C(1) << i), digits[i]));
  predicates[2] = ixs_cmp(ctx, key, IXS_CMP_EQ, ixs_int(ctx, 0));
...
static void test_exact_mod_range_refines_congruence(void) {
  ixs_ctx *ctx = get_ctx();
  ixs_node *major = ixs_sym(ctx, "mod_reverse_major");
  ixs_node *minor = ixs_sym(ctx, "mod_reverse_minor");
  ixs_node *major_mod_64 = ixs_mod(ctx, major, ixs_int(ctx, 64));
  ixs_node *major_digits =
      ixs_add(ctx, ixs_mod(ctx, major, ixs_int(ctx, 2)),
              ixs_add(ctx,
                      ixs_mul(ctx, ixs_int(ctx, 2),
                              ixs_mod(ctx,
                                      ixs_floor(ctx, ixs_div(ctx, major_mod_64,
                                                             ixs_int(ctx, 2))),
                                      ixs_int(ctx, 2))),
                      ixs_mul(ctx, ixs_int(ctx, 4),
                              ixs_mod(ctx,
  ...
  ixs_node *predicates[] = {
      ixs_cmp(ctx, major, IXS_CMP_GE, ixs_int(ctx, 0)),
      ixs_cmp(ctx, major, IXS_CMP_LE, ixs_int(ctx, 511)),
      ixs_cmp(ctx, active, IXS_CMP_EQ, ixs_int(ctx, 0)),
  };
  ixs_node *minor_digits[] = {
      ixs_mod(ctx, minor, ixs_int(ctx, 2)),
      ixs_mod(ctx, ixs_floor(ctx, ixs_div(ctx, minor, ixs_int(ctx, 2))),
              ixs_int(ctx, 2)),
      ixs_mod(ctx, ixs_floor(ctx, ixs_div(ctx, minor, ixs_int(ctx, 4))),
              ixs_int(ctx, 2)),
  };
  ixs_facts *facts = ixs_facts_create(ctx);
  size_t i;
  ...
  CHECK(ixs_facts_assume_preds(facts, predicates, 3));
  for (i = 0; i < sizeof(minor_digits) / sizeof(minor_digits[0]); i++)
    CHECK(test_ixs_check_facts(facts, ixs_cmp(ctx, minor_digits[i], IXS_CMP_EQ,
                                              ixs_int(ctx, 0))) ==
          IXS_CHECK_TRUE);

  /* A remainder interval containing two residues does not fix congruence. */
  {
    ixs_node *loose_predicate =
        ixs_cmp(ctx, minor_mod_8, IXS_CMP_LE, ixs_int(ctx, 1));
    ixs_facts *loose = ixs_facts_create(ctx);
    CHECK(ixs_facts_assume_pred(loose, loose_predicate));
...
static ixs_node *test_mixed_radix_modular_form(ixs_ctx *ctx, ixs_node *scale,
                                               ixs_node *major,
                                               ixs_node *minor) {
  ixs_node *major_mod_64 = ixs_mod(ctx, major, ixs_int(ctx, 64));
  ixs_node *minor_mod_8 = ixs_mod(ctx, minor, ixs_int(ctx, 8));
  ixs_node *value =
      ixs_mul(ctx, ixs_int(ctx, 4),
              ixs_mul(ctx, scale,
                      ixs_floor(ctx, ixs_div(ctx, major, ixs_int(ctx, 64)))));

  value = ixs_add(
      ctx, value,
      ixs_mul(ctx, scale,
              ixs_floor(ctx, ixs_div(ctx, major_mod_64, ixs_int(ctx, 16)))));
  value = ixs_add(
      ctx, value,
      ixs_mul(ctx, ixs_int(ctx, 16),
              ixs_mul(ctx, scale,
                      ixs_floor(ctx, ixs_div(ctx, minor, ixs_int(ctx, 8))))));
  value = ixs_add(
      ctx, value,
      ixs_mul(ctx, ixs_int(ctx, 2),
              ixs_floor(ctx, ixs_div(ctx, minor_mod_8, ixs_int(ctx, 2)))));
  value = ixs_add(
      ctx, value,
      ixs_mul(ctx, ixs_int(ctx, 8), ixs_mod(ctx, major, ixs_int(ctx, 2))));
  value = ixs_add(ctx, value, ixs_mod(ctx, minor, ixs_int(ctx, 2)));
  value = ixs_add(ctx, value,
                  ixs_mul(ctx, ixs_int(ctx, 64),
                          ixs_mod(ctx,
                                  ixs_floor(ctx, ixs_div(ctx, major_mod_64,
                                                         ixs_int(ctx, 8))),
                  ...
                  ixs_mul(ctx, ixs_int(ctx, 16),
                          ixs_mod(ctx,
                                  ixs_floor(ctx, ixs_div(ctx, major_mod_64,
                                                         ixs_int(ctx, 2))),
                                  ixs_int(ctx, 2))));
  return ixs_mul(ctx, ixs_int(ctx, 8),
                 ixs_mod(ctx, ixs_mul(ctx, ixs_int(ctx, 2), value),
                         ixs_int(ctx, INT64_C(4294967296))));
}

static void test_predicate_equality_definition_normalization(void) {
  ixs_ctx *ctx = get_ctx();
  ixs_node *raw = ixs_sym(ctx, "predicate_normalize_raw");
  ixs_node *alias = ixs_sym(ctx, "predicate_normalize_alias");
  ixs_node *major = ixs_sym(ctx, "predicate_normalize_major");
  ixs_node *quotient = ixs_sym(ctx, "predicate_normalize_quotient");
  ixs_node *delta = ixs_sym(ctx, "predicate_normalize_delta");
  ixs_node *shifted_minor = ixs_sym(ctx, "predicate_normalize_shifted_minor");
  ixs_node *base_minor = ixs_sym(ctx, "predicate_normalize_base_minor");
  ixs_node *shifted =
      test_mixed_radix_modular_form(ctx, alias, major, shifted_minor);
  ixs_node *base = test_mixed_radix_modular_form(ctx, alias, major, base_minor);
  ixs_node *expected =
      ixs_add(ctx, ixs_mul(ctx, ixs_int(ctx, 16), delta), base);
  ixs_node *identity = ixs_cmp(ctx, shifted, IXS_CMP_EQ, expected);
  ixs_node *predicates[] = {
      ixs_cmp(ctx, raw, IXS_CMP_GE, ixs_int(ctx, INT32_MIN)),
      ixs_cmp(ctx, raw, IXS_CMP_LE, ixs_int(ctx, INT32_MAX)),
      ixs_cmp(ctx, major, IXS_CMP_GE, ixs_int(ctx, 0)),
      ixs_cmp(ctx, major, IXS_CMP_LE, ixs_int(ctx, 255)),
      ixs_cmp(ctx, quotient, IXS_CMP_GE, ixs_int(ctx, 0)),
      ixs_cmp(ctx, quotient, IXS_CMP_LE, ixs_int(ctx, 7)),
      ixs_cmp(ctx, delta, IXS_CMP_GE, ixs_int(ctx, 0)),
      ixs_cmp(ctx, delta, IXS_CMP_LE, ixs_int(ctx, 7)),
      ixs_cmp(ctx, shifted_minor, IXS_CMP_EQ,
              ixs_add(ctx, ixs_mul(ctx, ixs_int(ctx, 8), quotient), delta)),
      ixs_cmp(ctx, base_minor, IXS_CMP_EQ,
              ixs_mul(ctx, ixs_int(ctx, 8), quotient)),
      ixs_cmp(ctx, ixs_mod(ctx, raw, ixs_int(ctx, 16)), IXS_CMP_EQ,
              ixs_int(ctx, 0)),
      ixs_cmp(ctx, alias, IXS_CMP_EQ, raw),
  };
  ixs_facts *facts = ixs_facts_create(ctx);

  CHECK(ixs_facts_assume_preds(facts, predicates,
                               sizeof(predicates) / sizeof(predicates[0])));
  CHECK(test_ixs_check_predicate_facts(facts, identity) == IXS_CHECK_TRUE);

  /* At the next aligned displacement the residue wraps, so the unwrapped
   * affine RHS is not equal to the canonical Mod result. */
  {
    ixs_node *modulus = ixs_int(ctx, INT64_C(4294967296));
    ixs_node *wrap_point = ixs_mul(
        ctx, ixs_int(ctx, 8),
        ixs_mod(ctx, ixs_mul(ctx, ixs_int(ctx, 2), ixs_add(ctx, alias, delta)),
                modulus));
    ixs_node *wrap_base =
        ixs_mul(ctx, ixs_int(ctx, 8),
                ixs_mod(ctx, ixs_mul(ctx, ixs_int(ctx, 2), alias), modulus));
    ixs_node *wrap_expected =
        ixs_add(ctx, ixs_mul(ctx, ixs_int(ctx, 16), delta), wrap_base);
    ixs_node *wrap_identity =
        ixs_cmp(ctx, wrap_point, IXS_CMP_EQ, wrap_expected);
    ixs_node *wrapping_predicates[] = {
        ixs_cmp(ctx, raw, IXS_CMP_EQ, ixs_int(ctx, INT64_C(2147483632))),
        ixs_cmp(ctx, delta, IXS_CMP_EQ, ixs_int(ctx, 16)),
        ixs_cmp(ctx, ixs_mod(ctx, raw, ixs_int(ctx, 16)), IXS_CMP_EQ,
                ixs_int(ctx, 0)),
        ixs_cmp(ctx, alias, IXS_CMP_EQ, raw),
    };
    ixs_facts *wrapping = ixs_facts_create(ctx);

    CHECK(ixs_facts_assume_preds(wrapping, wrapping_predicates,
                                 sizeof(wrapping_predicates) /
                                     sizeof(wrapping_predicates[0])));
    CHECK(test_ixs_check_predicate_facts(wrapping, wrap_identity) ==
          IXS_CHECK_FALSE);
  }
}
...
int main(void) {
#ifdef IXS_TEST_INTERNAL
  test_exact_quotient_common_denominator();
#endif
  test_add_canonicalize();
  test_scaled_add_constant_cancellation();
  ...
  test_mod_representative_definition_proof();
...
#ifdef IXS_TEST_INTERNAL
  test_try_subs_arithmetic_contract();
#endif
  ...
  test_fact_mod_congruence_bounds();
  test_affine_endpoint_refinement();
  test_partitioned_affine_endpoint_refinement();
  test_exact_mod_range_refines_congruence();
  test_predicate_equality_definition_normalization();
